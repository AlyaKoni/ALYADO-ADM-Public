...
else
{
    ...
    try
    {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            $installString = "msiexec.exe /i `"$($toInst.FullName)`" /qn /norestart /L* `"C:\ProgramData\AlyaConsulting\Logs\AcroReadAipPlugin-Install-MSI-$AlyaTimeString.log`" ALLUSERS=1"
