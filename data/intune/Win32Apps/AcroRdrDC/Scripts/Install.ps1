...
else
{
    ...
    try
    {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            cmd /c "msiexec.exe /i `"$($toInst.FullName)`" TRANSFORMS=`"$($toTransform[0].FullName)`" /qn /norestart /L* `"C:\AlyaConsulting\Logs\AcroRdrDC-Install-Msi-$AlyaTimeString.log`" ALLUSERS=1"
            do
            {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            cmd /c "msiexec.exe /update `"$($toInst.FullName)`" /qn /norestart /L* `"C:\AlyaConsulting\Logs\AcroRdrDC-Install-Msp-$AlyaTimeString.log`" ALLUSERS=1"
            do
            {
                Start-Sleep -Seconds 5
                $process = Get-Process -Name "msiexec.exe" -ErrorAction SilentlyContinue
