...
else
{
    ...
    try
    {
        ...
        #Members
        $serviceLocation = (Get-Content (Join-Path $AlyaScriptDir "ServiceLocation.txt") -Raw -Encoding UTF8).Trim()
        ...
        foreach($toInst in $toInstall)
        {
            ...
            $installString = "msiexec.exe /i `"$($toInst.FullName)`" /qn /norestart /L* `"C:\ProgramData\AlyaConsulting\Logs\AzInfoProtection-Install-$AlyaTimeString.log`" ALLUSERS=1 ENABLETELEMETRY=0 ServiceLocation=$serviceLocation"
