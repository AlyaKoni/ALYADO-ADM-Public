...
else
{
    ...
    try
    {
        ...
        foreach($reg in (Get-ChildItem -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Installer\UserData\S-1-5-18\Products"))
        {
            $displayName = $null
            $publisher = $null
            $uninstallString = $null
            try {
                $displayName = Get-ItemPropertyValue -Path ($reg.PSPath+"\InstallProperties") -Name "DisplayName" -ErrorAction SilentlyContinue
            } catch {}
            try {
            ...
            if ($displayName -eq "Microsoft Purview Information Protection" -and $publisher -eq "Microsoft Corporation")
            {
				...
				try {
					$uninstallString = Get-ItemPropertyValue -Path ($reg.PSPath+"\InstallProperties") -Name "QuietUninstallString" -ErrorAction SilentlyContinue
				} catch {}
                if (-Not $uninstallString)
                {
                    $uninstallString = (Get-ItemPropertyValue -Path ($reg.PSPath+"\InstallProperties") -Name "UninstallString") + " /qn /norestart" -replace "/I", "/X"
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDHozuQRS0pBw/p
# bE8v9SiHgOei8PnSDzVe4q/WK4hLAKCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIIE3fOVX
# 9n2XKTlvE5Pc/7OTgYQ9+q5pQBuj0/OWck81MA0GCSqGSIb3DQEBAQUABIICAIwK
# AdECXAso+aqPsagvnqEyRPjOeU7gIHPsjH/I51m143iu6ZJ5Lm/PxzcJRFk27sof
# O4XqdjlZOAXGWUkDIvMan5snwevDOL4rnI/YbfXJL5GHpmTlif1jjEWLJ5ezwRoq
# YaL+33Cr5nHBqmDPmaHVULBxcSfRnFNHKebIzydCaZQQ5Yu4YXvcQ0i+/Yt+dIXQ
# 8rYawADRhIvJEpWCwqiZPa4t1eTsdNfBghjVrBrRMyq9VmrkFrvfhVBkDx/61CsQ
# FWTJU0mE8Wt/cw63bhW9AoGq/dwtilZpGpvGVZGltGSzaJ6wrElhEWSkv5fs6ZjC
# g6X8GK2CyGU8o35u0MBBx0vd4ku+x196GvY7LSnciYp2njhgdll8JqFtJHbt2iXx
# 8cGJZC3wEfP3GA6QphO6+6VRefGC/aSTF7tq2i/S3yGg6nBGBG8cwEscw49eNBxW
# kf6EyDNk84lJ3M0PdlWpzm8k9WzprZQfCdODdg9Uy1pZvk0+ZE9CzJMcYSuAoZhU
# +zngJcuXbLXxGtIP/UmeYFaOiM1Vs+641KFSYNgprxvcs7xXeT2c5CCzDTZ+De50
# cIPo2hx0XO/SCdiDVZ/8a46LNfYPYvGRSwEhmuVxJQXc5l1Ara0zVvY9YzM7U1aP
# UOECnJE+h5bsa2FJcS+AoTfVR2pSrJVl9TELQYv2oYIWzTCCFskGCisGAQQBgjcD
...
# SAFlAwQCAQUABCDX72j2YMbKwGHenoQaB2H5G/wAsqoFEU5VB10rF1bHJwIUcXqa
# DNZ1jzZdLXz8glVwRU7SlYEYDzIwMjUwMjA2MTg0ODE5WjADAgEBoGGkXzBdMQsw
...
# hvcNAQELBQAwLwYJKoZIhvcNAQkEMSIEIKP3fpaI2qTIWTmb7gf0aLS8mLAwIqsi
# 7P1QzoREo4vhMIGwBgsqhkiG9w0BCRACLzGBoDCBnTCBmjCBlwQgOoh6lRteuSpe
...
# KoZIhvcNAQELBQAEggGA57KdbBsvyQRdlAyxuePQX5yBFvWKMl8rKvLNTm5w7CGI
# z2Mbcz5xBz9DGQ7wZRCWSqTz0+6UWUEqrxfaAd7OJZL/fn/zv4cAVa0hW7KoLj6D
# gCc9DM9Lnrnm2jP/HFkW+0J+01ymILmgt+wUdpqrek9EmDPyMqxiXmLWBHgPoupZ
# fyKWXEXiZKMhzrnNX8jAz+DjfGfFJSbFfo8lmkFylPqEwewyhpgaCoR8n8HMj+Ai
# jo7PJtM+U5UbJ3PDmW8tOrdOcM2nAkA+xJ/2ixf/a2AuzRyvu2NJ0/UKLbFccwBT
# bTTLblXjhoCFk74mUXaUh5gkQKrw/QJu3PYLwpy8gOycf12cxq/ejkguzTj5nlH0
# uEX9rw631lUZzv5/Y2izo5bZY0mFndzC7/arroMfpVacMyP0Ks7eeGfcbfubZGSK
# XXm7L9IIUctQJCZHx8qfgN64k48F4iEIh797vR3haBzhQWOwOGFlAORceYKvdkRl
# MWFq/RPnjyhQNUMT3XYC
