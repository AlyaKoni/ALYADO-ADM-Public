    ...
    try
    {
        ...
        foreach($regPlat in $regPlats)
        {
            foreach($reg in (Get-ChildItem -Path "HKLM:\SOFTWARE$regPlat\Microsoft\Windows\CurrentVersion\Uninstall"))
            {
                ...
                if ($displayName -like "GRAPHISOFT BIMx Desktop Viewer" -and $publisher -eq "GRAPHISOFT SE")
                {
                    ...
                    $uninstallString += " /L* `"C:\ProgramData\AlyaConsulting\Logs\BIMxDesktop-Uninstall-$AlyaTimeString.log`""
