...
else
{
    ...
    try
    {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            cmd /c "msiexec.exe /i `"$($toInst.FullName)`" /qn /norestart /L* `"C:\AlyaConsulting\Logs\Firefox-Install-$AlyaTimeString.log`" ALLUSERS=1 ENABLETELEMETRY=0 DESKTOP_SHORTCUT=false PREVENT_REBOOT_REQUIRED=true"
            do
            {
