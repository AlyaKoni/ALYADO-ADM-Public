...
<#
.SYNOPSIS
Ensures execution in a 64-bit PowerShell environment and installs all MSI packages located in the same directory as the script while generating detailed logs.

.DESCRIPTION
The Install.ps1 script verifies whether it is running in a 64-bit PowerShell process. If not, it restarts itself using the 64-bit PowerShell executable. Once running in 64-bit mode, it logs execution information and installs all MSI files found in the same directory. Each installation is logged with timestamps and monitored to ensure all msiexec processes complete before continuing. The script also captures errors and writes them to the log. 

.INPUTS
None. The script does not accept input from the pipeline.

.OUTPUTS
Integer. Returns 0 on success or a non-zero exit code if an error occurs during execution or installation.
...
else
{
    ...
    try
    {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            $installString = "msiexec.exe /i `"$($toInst.FullName)`" /qn /norestart /L* `"C:\ProgramData\AlyaConsulting\Logs\Firefox-Install-$AlyaTimeString.log`" ALLUSERS=1 ENABLETELEMETRY=0 DESKTOP_SHORTCUT=false PREVENT_REBOOT_REQUIRED=true"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDOjRCLlHrN+heC
# WjkvrqaXQtJhV2wyj0XT92ArcEY1FqCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEICh77eQDD7DuoWyw
# KzDsvQ5vfm1EPqCZU0g68tIDL+PIMA0GCSqGSIb3DQEBAQUABIICAKNX7o0PDiDZ
# gpY35ePkPmYL5S46On2YJUMs0DRGk/dldcOjQaLyFD6OSQ9e1ppPZPWH19KpbR8y
# UOcA27ud1LaPUPqkXHJZiyqYsqazQZKbAi7An3yN5J/Uc+iQQdhnaWRLk8r4Y4A7
# kSl4hZAmwSPpq1lUPzWHBcOmlfwM/vd0fr6H0PR1StP+m22u4IBRHz+Pak5CKpwH
# xwGDP9olFtvz4hsSRGj12pZwM1DKGJnvCpegt5RsqyfF+5rpLc0Ah1YA565lnxsV
# fR0R06lSs4ZGQuA4MU/e+b1hjljbBzYHkwgnAnICh0i62vywan24aZpHHeRUp1ZH
# IPm3WJxWZr8U6WsWJTahz/NJbfk7nJoye7RexGxEnfgrxzixcUZMX75tkY3TFxSD
# 8JYTDjEfknfQgCQlT9D+Y7D6pJ5d4tbb4q9iFk/bedunOTYXjO4nc5Y35hFG54xw
# PJNf+VRLt3CfWVKNLnVk9mo3wrwCSkWn0d1rnbyHjsJnaDIHu4xLtE4ZYeRLtRv+
# cs4Z5uGqcdRoTH9EY10AO+OHwWb6rkfr9oPjXzRVsrDc/jbtspVgouI826A5aHUD
# kWhK+NYlu0/C7USMXQWwHKaWlEO/z3BDKdnFS3WBoB/2g5mc2y6A5bKCn3SSD6bm
# 2gG2haDk4IfpM4PbRO9dtN4lyE8kulgFoYIWujCCFrYGCisGAQQBgjcDAwExgham
...
# AQUABCCmprCboHaPVRyP8v9uMt8mMTSJEnxlcappF1uARQX0gQITczeQW4nnSCoj
# 13RazGJQ0RGu9xgPMjAyNjAyMDYxMDA2MDlaMAMCAQGgWKRWMFQxCzAJBgNVBAYT
...
# z4NRFKrFrU54eaj6tr1JosFN6kj78iq0m7OSoqBzWQ0wgbAGCyqGSIb3DQEJEAIv
...
# NAIQAQALIAWzlAdi2z42eZSbqTANBgkqhkiG9w0BAQsFAASCAYAIQB42+Iwn+AjL
# 6AJ3k5YRRJm/Ve58EWmoAKZ463BQBMCYXaYfqa8chMqfoFR/bwv4QuyaXNwW3TJP
# erSMx0oYe+M7LiPIJxrvwqejPFiJIAN08ae7Ndq2TNvxCi9a8Pc4ftrZyBT2EuWd
# RYQqHytDKwkxs7vWlEFuzC0CYB31iZoDSflcLCzXXPLsbDaESuzipE8+0cM1xhba
# Q5qFwjJ5apRu0SzCju80fwj9Je1hGzh3qY1OUi+N0dMl7FU+o7JtSYavFMiW4TDc
# DQEFZwLEs17da/NMpc2Dhi553xEEYGfNVp2/fQDHWIQWGowuSxkH1CE9KhupL+XE
# 4kN6tkQZb85xMdjvd4ccMeMRzq+Bp3V0UE+TiaNnikKYW/Eog9jwBUcw0q1jZctM
# 7nx76UyTwGbGaykD7CtQn1GS9q9ZtXbqY/yz2ro/C+0z7EOyTWttcLoQNymuBk+o
# mY0Kq5OcvligRw60JVUumaElCtpfKqccM9cD4dvqlFcflqZ9Kvc=
