...
else
{
    ...
    try
    {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            start /wait "gitInstaller" "$($toInst.FullName)" /VERYSILENT /NORESTART /NOCANCEL /SP- /CLOSEAPPLICATIONS /RESTARTAPPLICATIONS /LOADINF="$($AlyaScriptDir)\GitInstallParameters.inf"
            #TODO wait for
