...
else
{
    ...
    try
    {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            $installString = "msiexec.exe /i `"$($toInst.FullName)`" /qn /norestart /L* `"C:\ProgramData\AlyaConsulting\Logs\iCloudApplSup-Install-Msi-$AlyaTimeString.log`" ALLUSERS=1"
        ...
        foreach($toInst in $toInstall)
        {
            ...
            $installString = "`"$($toInst.FullName)`" REBOOT=ReallySuppress /qn /norestart /L* `"C:\ProgramData\AlyaConsulting\Logs\iCloud-Install-Exe-$AlyaTimeString.log`" ALLUSERS=1"
            Write-Host "command: $installString"
            cmd /c "$installString"
			Write-Host "CMD returned: $LASTEXITCODE at $((Get-Date).ToString("yyyyMMddHHmmssfff"))"
            do
