...
if (-Not $driverVersion)
{
    ...
    $link = Invoke-WebRequest -Uri "https://www.nvidia.com/Download/processDriver.aspx?psid=$psid&pfid=$pfid&rpf=$rpf&osid=$osid&lid=$lid&lang=en-us&ctk=0" -Method GET -UseBasicParsing
    $link = Invoke-WebRequest -Uri $link.Content -Method GET -UseBasicParsing
    $link = ($link.Links | where { $_.outerHTML.Contains("lnkDwnldBtn") }).href
