...
else
{
    ...
    try
    {
        ...
        foreach($toInst in $toInstall)
        {
            ...
            cmd /c "msiexec.exe /i `"$($toInst.FullName)`" /qn /norestart /L* `"C:\AlyaConsulting\Logs\TortoiseGit-Install-$AlyaTimeString.log`" ALLUSERS=1 ENABLETELEMETRY=0 ServiceLocation=$serviceLocation"
            do
            {
