...
Start-Transcript -Path "$($AlyaLogs)\data\wvd\admin\fall2019test\15_createOrUpdateAppGroups_hpol002-$($AlyaTimeString).log" | Out-Null
...
$appsToGroup = @(@("Standard Apps",@("Access","Excel","OneDrive","OneNote2016","Outlook","PowerPoint","Word","Explorer","GoogleChrome","IrfanView64453","Pdf24","Taskmanager","AcrobatReader2017","RemoteDesktopConnection","Teams","AgentRansack","Firefox","TinyPicExe","WinRar","Notepad"),@("WVDTAPPGRP_Standard")),`
                 @("Visio App",@("Visio"),@("WVDTAPPGRP_Visio")),`
                 @("Project App",@("Project"),@("WVDTAPPGRP_Project")),`
                 @("Abacus App",@("AbacusAbaStart"),@("WVDTAPPGRP_Abacus")),`
                 @("Adobe Apps",@("AdobeCreativeCloud"),@("WVDTAPPGRP_Adobe")))
...
& "$($AlyaScripts)\wvd\admin\fall2019test\15_createOrUpdateAppGroupsApp.ps1" -HostPoolName $HostPoolName -appDefs $appDefs -appsToGroup $appsToGroup -availableIcons $availableIcons
