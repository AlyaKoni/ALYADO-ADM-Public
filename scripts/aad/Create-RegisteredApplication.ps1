...
Param(
    $ApplicationName = "ExchangeIface",
    $AssignKeyAuth = $true,
    $AssignCertAuth = $true,
    $RequiredResourceAccess = $null,
    $SetRequiredResourceAccessForContacts = $false,
    $SetRequiredResourceAccessForGuestManagement = $true,
    $SetExchangeRbacForUPN = "administration@docuvita.swiss"
)
...
if (-Not $ApplicationName.StartsWith($AlyaNamingPrefix))
{
...
if (-Not $AzAdApplication)
{
    ...
    $KeyId = [Guid]::NewGuid()
    $AzAdApplication = New-AzADApplication -DisplayName $ApplicationName -IdentifierUris ("http://$AlyaTenantName/$KeyId")
    $AzAdServicePrincipal = New-AzADServicePrincipal -ApplicationId $AzAdApplication.AppId
}
else
{
    $AzAdServicePrincipal = Get-AzADServicePrincipal -DisplayName $ApplicationName
}

# Merging ressource access
Write-Host "Merging ressource access" -ForegroundColor $CommandInfo
$MgApp = Get-MgBetaApplication -ApplicationId $AzAdApplication.Id -Property "RequiredResourceAccess"
    ...
    foreach($AppPermission in $MgApp.RequiredResourceAccess)
    {
        ...
        if ($ExistingAppPermission)
        {
            foreach($access in $AppPermission.ResourceAccess)
            {
                ...
                {
                    Write-Host "  $access permission already exists"
                }
                else
                {
                    Write-Host "  Adding $access permission"
...
if ($AssignCertAuth)
{
    try
    {
        ...
        $AppCredential = Get-AzADAppCredential -ApplicationId $AzAdApplication.AppId -ErrorAction SilentlyContinue | Where-Object { $_.Type -eq "AsymmetricX509Cert" -or $_.Type -eq "Symmetric" }
...
if ($AssignKeyAuth)
{
    ...
    $AppCredential = Get-AzADAppCredential -ApplicationId $AzAdApplication.AppId -ErrorAction SilentlyContinue | Where-Object { [Convert]::ToBase64String($_.CustomKeyIdentifier) -eq $keyIdentifier }
    if (-Not $AppCredential)
    {
        Write-Host "  Not found" -ForegroundColor $CommandWarning
	    $startDate = (Get-Date)
	    $endDate = (Get-Date).AddYears(2)
	    $AppCredential = New-AzADAppCredential -ApplicationId $AzAdApplication.AppId -CustomKeyIdentifier $keyIdentifier -StartDate $startDate -EndDate $endDate
...
if ($SetExchangeRbacForUPN)
{
    ...
    if (-Not $ExServPrinc)
    {
        ...
        $ExServPrinc = New-ServicePrincipal -AppId $AzAdApplication.AppId -ObjectId $AzAdServicePrincipal.id -DisplayName $ApplicationName
        $ExServPrinc = Get-ServicePrincipal -Identity $ApplicationName -ErrorAction SilentlyContinue
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA5ayx3YhsKII7y
# A7VXIhP/09xQe96LUwwWMV9MSzLluaCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIFOeCLTU8IvWjJ6g
# NxtuvQzRZ1gX6nxAhV9oUN9cMe83MA0GCSqGSIb3DQEBAQUABIICAHohIhiAa+CP
# FVCiCSuRpreMOTc9TuGACMmzlc1kroA289Lhfa7poKuphR4lH7Kibel/2t9p2kD6
# boAXJT8/lGDS/bhQYeaqqrJSTJU/3q+E5fjKckcIBBnDFDNgfKTDkSUUIoBMUSl1
