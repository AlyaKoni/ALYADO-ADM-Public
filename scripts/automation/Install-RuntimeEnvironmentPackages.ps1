...
<#
.SYNOPSIS
Installs the required PowerShell modules into Azure Automation runtime environments by interacting with Azure and the PowerShell Gallery.

.DESCRIPTION
The Install-RuntimeEnvironmentPackages.ps1 script automates the validation and installation of PowerShell module dependencies into specified Azure Automation runtime environments. It connects to Azure, verifies the existence of required resources such as the resource group and automation account, retrieves runtime environment configurations, and ensures that all listed packages are installed or updated. It interacts directly with the Azure REST API to check and manage packages, supports processing specific runtime environments, and logs all actions performed. The script is designed for use within the Alya Base Configuration framework and assumes prior environment setup.

.PARAMETER Language
Specifies the scripting language of the runtime environment to process. Default is "PowerShell".

.PARAMETER ProcessOnlyRunTimeEnvironment
Specifies a specific runtime environment to limit processing to. If null, all applicable runtime environments for the specified language are processed.

.PARAMETER PackagesToInstall
Defines a list of PowerShell modules to check and install in the runtime environment. If the Version property is null, the latest version from the PowerShell Gallery is installed.

.INPUTS
None. The script does not accept pipeline input.

.OUTPUTS
The script produces console output indicating progress, status messages, and any errors encountered. The transcript is saved to a log file.

.EXAMPLE
PS> .\Install-RuntimeEnvironmentPackages.ps1 -Language "PowerShell" -ProcessOnlyRunTimeEnvironment "RunEnv01"
...
Param(
    ...
    [object]$PackagesToInstall = @( @{Name="PackageManagement"; Version=$null}, @{Name="PowerShellGet"; Version=$null}, @{Name="MSAL.PS"; Version=$null}, @{Name="Az.Accounts"; Version=$null}, @{Name="Az.Automation"; Version=$null}, @{Name="Az.KeyVault"; Version=$null}, @{Name="Az.Resources"; Version=$null}, @{Name="Az.Storage"; Version=$null}, @{Name="ExchangeOnlineManagement"; Version=$null}, @{Name="MicrosoftTeams"; Version=$null}, @{Name="Microsoft.Online.SharePoint.PowerShell"; Version=$null}, @{Name="PnP.PowerShell"; Version=$null} , @{Name="AzTable"; Version=$null} ) #Version $null means latest
)
...
Write-Host "Automation | Install-RuntimeEnvironmentPackages | AZURE" -ForegroundColor $CommandInfo
...
foreach($runEnv in $runEnvs)
{
    ...
    foreach($package in $packages)
    {
        ...
        foreach ($packageToInstall in $PackagesToInstall)
        {
            ...
            if ($package -eq $packageToInstallName)
            {
                Write-Warning "Package $packageToInstallName is already installed in this runtime environment"
                $allPackages = $allPackages | Where-Object { $_.Name -ne $packageToInstallName }
            }
        }
    }
    ...
    foreach ($packageToInstall in $allPackages)
    {
        ...
        {
            Write-Host "Installing package $packageName" -ForegroundColor $CommandInfo
        }
        else
        {
            Write-Host "Installing package $packageName with version $packageVersion" -ForegroundColor $CommandInfo
        }
        ...
        do
        {
            ...
            $Url = "https://www.powershellgallery.com/api/v2/Search()?`$filter={1}&searchTerm=%27$packageName%27&targetFramework=%27%27&includePrerelease=false&`$skip=0&`$top=40"
            $Url =
                if ($packageVersion) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCApWSZW9NSg89Fx
# 9g1QeWkK+V03Si3n5as2EoQ7DUJmWaCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEI