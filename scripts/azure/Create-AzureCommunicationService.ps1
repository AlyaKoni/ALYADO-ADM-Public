...
<#
.SYNOPSIS
Creates and configures an Azure Communication Service, with optional connection to an Azure Communication Email Service and SMTP role setup.

.DESCRIPTION
The Create-AzureCommunicationService.ps1 script provisions an Azure Communication Service within a specified Azure environment. It validates prerequisites such as configuration variables, resource provider registration, resource group existence, and key vault availability. If configured, it also connects the communication service to an existing Azure Communication Email Service, creates or verifies required Azure AD applications, custom roles, and role assignments for SMTP functionality. The script logs its actions, manages key vault permissions, and provides SMTP connection details for testing and use.

.PARAMETER ConnectCommunicationEmailService
Indicates whether the script should connect the Communication Service with the Communication Email Service and configure SMTP-related components. Defaults to $true.

.INPUTS
None. The script reads all required configuration from external configuration files and Azure environment variables.

.OUTPUTS
The script outputs status messages to the console and generates a transcript log file in the defined logs directory. It also updates Azure resources as part of the provisioning process.

.EXAMPLE
PS> .\Create-AzureCommunicationService.ps1 -ConnectCommunicationEmailService $true

Creates and configures an Azure Communication Service and connects it with the Communication Email Service for SMTP use.
...
"@
	...
	$AzAdApplication = Get-AzADApplication -DisplayName "$($AlyaCompanyNameShortM365)CommunicationServiceSmtp" -ErrorAction SilentlyContinue
	if (-Not $AzAdApplication)
	{
	    ...
	    $AzAdApplication = New-AzADApplication -DisplayName "$($AlyaCompanyNameShortM365)CommunicationServiceSmtp" -HomePage $HomePageUrl -IdentifierUris ("http://$AlyaTenantName/$KeyId")
	    $AzAdServicePrincipal = New-AzADServicePrincipal -ApplicationId $AzAdApplication.AppId
	
	    #Create credential 
	    $startDate = (Get-Date)
	    $endDate = (Get-Date).AddMonths(120)
	    $AlyaServicePrincipalSecret = New-AzADAppCredential -ApplicationId $AzAdApplication.AppId -CustomKeyIdentifier ([Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes("KeyVault $KeyVaultName"))) -StartDate $startDate -EndDate $endDate
	...
	{
	    $AzAdServicePrincipal = Get-AzADServicePrincipal -DisplayName "$($AlyaCompanyNameShortM365)CommunicationServiceSmtp" | Where-Object { $_.AppId -eq $AzAdApplication.AppId }
	}
...
if ($ConnectCommunicationEmailService)
{
	...
	foreach($emlSender in $senders)
	{
	    Write-Host "    $($emlSender.Name)@$($domain)"
	}
	...
	$RoleAssignment = Get-AzRoleAssignment -RoleDefinitionName "$($AlyaCompanyNameShortM365)CommunicationServiceSmtp" -ObjectId $AzAdServicePrincipal.Id -Scope $CommService.Id -ErrorAction SilentlyContinue | Where-Object { $_.Scope -eq $CommService.Id }
	$Retries = 0;
	While ($null -eq $RoleAssignment -and $Retries -le 6)
	{
	    $RoleAssignment = New-AzRoleAssignment -RoleDefinitionName "$($AlyaCompanyNameShortM365)CommunicationServiceSmtp" -ObjectId $AzAdServicePrincipal.Id -Scope $CommService.Id -ErrorAction SilentlyContinue
	    Start-Sleep -s 10
	    $RoleAssignment = Get-AzRoleAssignment -RoleDefinitionName "$($AlyaCompanyNameShortM365)CommunicationServiceSmtp" -ObjectId $AzAdServicePrincipal.Id -Scope $CommService.Id -ErrorAction SilentlyContinue | Where-Object { $_.Scope -eq $CommService.Id }
	    $Retries++;
	}
	if ($Retries -gt 6)
	{
	    throw "Was not able to set role assigment '$($AlyaCompanyNameShortM365)CommunicationServiceSmtp' for app $($AzAdServicePrincipal.Id) on scope $($CommService.Id)"
	}
	
	# Creating username
    Write-Host "Creating username" -ForegroundColor $CommandInfo
    Write-Warning "PowerShell has no possibility to create an alias for the username. Please use the Azure Portal to create an alias for the username $($senders[0].Name)@$($domain) in the Communication Service $CommServiceName"

	# Done
	$username = "$CommServiceName|$($AzAdServicePrincipal.AppId)|$AlyaTenantId"
	...
	Write-Host "  Username Alias: $($senders[0].Name)@$($domain)"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCxElS2UX0IhWZI
# PS9StedpOcs3LCSCtaze/U8F8Jc+l6CCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIIuf+z3r
# +MgWrfgkddHeH5DRX7h+3FMzoTX6plKlPkboMA0GCSqGSIb3DQEBAQUABIICAJkM
# txmReNfl8EadJduReZTfd/HTASPLsvF9d3UUNnMMqjzDVoH7CXzrqaq4943EBfCV
# kRloEeJHqCkUHCHi/773Dh2L7VDFbmdkgTRH6VwYER1MU/L3pJtjSTNsH7hrFIYH
# oF3FbVRFzUKqTgqaPBL+ftuvDqaJWvdxi5yVpXd3XpOqmL4qeqh2tuei695wRbGd
# q0r1839XZa/bwcn0N7vj6eaIwNuKswhTvOWki9D3uwklnNzYPHtNucc7M165lOmD
# Wm0/OFNtoWoppaZNxrjyMLfj6T8vZQBrVko3j6hFONZTw7p9z8cIemWhPolBan25
# MYPEomiWo1a6SLxcTtUU3fQPI3wvxOQWYgARbmkHYu49gJd4E2r6+mnn+5buj03S
# Nro7P1b58+VYr7SdIAFovfD3a6tFbblgI0m4O35qjzjZtAdGXRK1v57JRfYg/xOV
# 7gfRkSgFrZFp+RA1Gsol9OWo2k/4mynbzB/9S20CbK3rpxRz13X4R/9q05svIMAb
# B7baMT+pQBb9jt/hYuS+jFRyHNf3J32SJMol9KMtPYE4mPpf/AA9Dyq2LJ19ouoK
# wV99eCOrdL/DYTXCZ2NoRwICPnpTEnxW3XZyUvBAetJs7UL+sV4nx0pMC7/SUVvT
# 4Hzq6GTU7D9ghDQGfc2CiuYLF09B+h4Xy05Smku1oYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCCwIOReM1nlZRmr8AiGCIdzwPNpxsghUI+8pHvN5IkIlAIUGwuR
# F1tCh2UrtWjKsUXJbo0wVzcYDzIwMjYwMjEyMjAzNDA5WjADAgEBoFikVjBUMQsw
...
# AQkEMSIEIH4lJRn/kaRzApnO1Yk5HYNP1cd3gJQitV0vqFrOdhdHMIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAbqmL
# XRS7XxuJNFxgxHi7M6gkpIIWsRDqeerm6yTntLUHmxKeQsZfQ+Bxcef4a75YLDJX
# 6AkCaKDRNhxSoVOVe7TkyKdg5F7+bgZZg1rFlbbt8cmjNLQqiUGIVkilUnZymxoU
# LRpcyK6sHQtDL3ZSE1jUSLbQKG0GphEjHzX7gQKEZEXzNXiYCUsiaIs9LB0uMg10
# aCm8bDX9ynoqbAfmUmn+QV/9Fbe1bDdgmTqwfISUZk3y/Us60Bm2Cjr+0M8FssN3
# 0xWrFWP5XtKIp9IrcrxyYLWjG8CsuAJGrPCvtFy/KNGLEXFDOS1qFSbRXdslYwZH
# 0daQ0/WaDhX7k56gIeJNODcZrkgbqRiZKYG8vv7ZT+B+kkNqfHyxzd/l64HGjv0F
# KL9M20iT0L5HPOYdM3MLbu0UD+JMeqzs9ua3L/ps0+49sf1xu90tNYLnqSkEfVN+
# Qr01IUJwbffIocUoG937dBEGymFdA9Us8M7NBSiD3CvipCtTas0Jawd8dRpv
