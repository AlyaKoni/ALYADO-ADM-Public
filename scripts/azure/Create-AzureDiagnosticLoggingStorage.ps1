...
<#
.SYNOPSIS
Creates or validates an Azure Storage Account used for diagnostic logging within the Alya Base Configuration environment.

.DESCRIPTION
The Create-AzureDiagnosticLoggingStorage.ps1 script automates the setup and validation of an Azure Storage Account intended for the collection of diagnostic logs. It ensures that required Azure modules are installed, logs in to the appropriate subscription, checks that the required Microsoft.Storage resource provider is registered, and creates the necessary resource group and storage account if they do not exist. The script also applies lifecycle management settings to the storage account by calling a dedicated retention setup script. All operations and outputs are logged in a transcript file.

.INPUTS
None. The script relies on predefined variables and configuration sourced from 01_ConfigureEnv.ps1.

.OUTPUTS
Creates or updates Azure resources including a resource group and a storage account for diagnostic logging. Outputs log information to a transcript file.

.EXAMPLE
PS> .\Create-AzureDiagnosticLoggingStorage.ps1
Executes the script to check for and create if necessary the Azure resource group and storage account for diagnostic logging.
...
if (-Not $StrgAccount)
{
    ...
    $StrgAccount = New-AzStorageAccount -Name $StorageAccountName -ResourceGroupName $ResourceGroupName -Location $AlyaLocation -SkuName "Standard_LRS" -Kind StorageV2 -MinimumTlsVersion "TLS1_2" -AccessTier Cool -Tag @{displayName="Diagnostic Log Storage";ownerEmail=$Context.Account.Id}
    if (-Not $StrgAccount)
    {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBV6NDLfZNGk9ow
# GboxMRmsP2fOpfYIc3rC3RyvIE9wuaCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIGDv41H3PZYfRsqm
# e1tFV/nCFuQ+XpyVi6CB0jxb7AdHMA0GCSqGSIb3DQEBAQUABIICAGo9CJ/vL6GP
# aPWbB9VDfcZF/J7zyNQQ0em5njjtIP1bCnAGi4tm87zkvGaLBdbTtdc+XfiqezXg
# YCoj6qDJQbiS/bxi8C+RLtrkwe+TCVF7xIsvV1MWT9x7c4WX4cCYbEpBOimV5vOm
# YrNJg+w0KJkjPIlPzV8LElgs0X4ayd1YLX30/UQ+3d16BmMhYbaElsL9rXpZiSh/
# KQNLZiyuUNmJdPgqmCpSTzye+tlVOUa4opMuxILGKHw5Zc9UPPpE9ac/VRF6oYcf
# HVGpe+TnL/7WrpFXtqckvx+i8hvwWzkkge2qjYy6Zs32ImpRqeajR2peJM69O9U1
# ZGg7CDqHGmOVRoRYxfPx14m1AFWmxkainBxI80bEX78xYjDtimLBMexmRYnqoTGQ
# CEdzWk+6bqPCHJ+AJ9sVx3d495Lx5jYjxQPoBhP+Q4q52id6nLoqmG4c0rytD7vr
# I+vS+DB8XxEzattapswmcRqrv/KdQ7uBL34S7HKLZUPOQ9HdsMmZTZ2VHv+I00Ot
# dtkXoIEauvasGfBkV2LSMgjMX5RBV28amAQz6t+t1PSjS6GHm16qqMXBytUWDDgO
# wMkOXykJkHx9BYXWdjhEy2YH2zzJxC8tdD9setnJs0v63mA4M5ljhTbtWojMV7eX
# rywlL94Tsxid5jPZXPlCnYi+rrgrOyD7oYId7TCCHekGCisGAQQBgjcDAwExgh3Z
...
# AQUABCC9MQUeKt+nwobrjY/LAXZf7Gl7qh5iEs4GlB2Mzr8NwAIULmvmvh2afZhg
# Jm9bGpejfVbuAboYDzIwMjYwNTE5MDczNjMyWjADAgEBoF2kWzBZMQswCQYDVQQG
...
# MBwwCwYJYIZIAWUDBAICoQ0GCSqGSIb3DQEBDAUAMD8GCSqGSIb3DQEJBDEyBDAT
# v4OFrmyKwICrrVYURqq7ig9SNwRcR+qu8obnEYpjvzt67zgWU6N3h5a42CaqVgUw
...
# AQEMBQAEggGAx8tW2Io7j829lyJFk1r2MiY9Mn8pAcYKYRTf9RMee0dGvyORlrUq
# ERlMMJrMEAeJFOPZpiVoIZgqHPD3cJBbBaBQNiMPc5aM7Gt1R+HkcV89BMKNErBr
# j7LhAYUov1HE+8qO6DRlAD3g9/MdbN6qI14gmz5NjzTtDLYYOs7FQZkAsT/ZF5td
# G8SDY58bTm9NOdQt7xxDTI4TjfgPDYf1+gO8+rPD1EqzNc979v+k0kpOzbwELggd
# KE/4v4qDPOnonQGkk1jY8OKWzG4chsgvF0u10BIfAR/HQheW20UPUA9UgHdVFFe5
# rrVyL8lRt940X1yp+Kz52p0/ZCj6ukX7dZKd6uVPkJiQTueeROlNEWPnm/EkXNyk
# IN7PmqnpX92Olpg7slYs3AC6V324bhGL5MRxOh4nEidmuCAUZuFLEBfyB//N7Elo
# c1YNshJk5MKtXJHRg6CPUfNB4PKKLEvGDyER1I6m/Woa+TSYBqyFtylRfCfkBUy6
# jiPvgRsJgaVF
