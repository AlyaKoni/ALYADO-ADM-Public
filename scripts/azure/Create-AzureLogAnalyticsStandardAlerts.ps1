...
<#
.SYNOPSIS
Creates and configures standard Azure Log Analytics alerts for a defined workspace.

.DESCRIPTION
The Create-AzureLogAnalyticsStandardAlerts.ps1 script automates the creation and management of standard alert rules in Azure Log Analytics workspaces. It verifies required modules, logs into Azure, checks the existence of required resources, and then programmatically creates and updates a predefined set of alerts (such as low disk space, high CPU usage, low memory, backup failures, and more) depending on the workspace configuration. Alerts are linked to a specific action group for notification handling. The script ensures the necessary role assignments and alert configurations are properly applied via Azure Monitor and Log Analytics APIs.

.INPUTS
None. The script uses configuration values sourced from the environment configuration file (01_ConfigureEnv.ps1).

.OUTPUTS
Log entries and alert rule configurations within the specified Azure Log Analytics workspace. The script also produces a transcript log file stored under the Alya logs directory.

.EXAMPLE
PS> .\Create-AzureLogAnalyticsStandardAlerts.ps1
...
function Create-Alert($Subscription, $AlertText,$AlertResourceGroupName,$LogAnaWrkspc,$ScheduledLogs,$ActionGroupId,$Severity,$QueryType,$ThresholdOperator,$Threshold,$FrequencyInMinutes,$UsesAzureResourceGraph = $false,$TimeWindowInMinutes,$Query)
{
  try {
    ...
    $queryStr = $Query
    if ($Query.StartsWith("arg(") -or $UsesAzureResourceGraph)
    {
...
function Prepare-StandardAlerts ($AlertSubscriptionName, $AlertResourceGroupName, $WrkspcResourceGroupName, $ActionGroupResourceGroupName, $ActionGroupName, $WrkspcName)
{
    ...
    Create-Alert `
        ...
        -FrequencyInMinutes 240 `
        -TimeWindowInMinutes 240 `
        -Query "let _minAvailableMB = 1024; let _timeWindow = 4h; let _linuxMem = Perf | where TimeGenerated >= ago(_timeWindow) | where CounterName == `"Available MBytes Memory`" | summarize mtgPerf=max(TimeGenerated), CounterValue=avg(CounterValue), SampleCount=count(CounterValue) by Computer | join kind=inner ( Heartbeat     | where OSType == `"Linux`" | summarize max(TimeGenerated) by Computer ) on Computer | project Computer, mtgPerf, AvailableMBytes=round(CounterValue), SampleCount; let _windowsMem = Perf | where TimeGenerated >= ago(_timeWindow) | where CounterName == `"Available MBytes`" | summarize mtgPerf=max(TimeGenerated), CounterValue=avg(CounterValue), SampleCount=count(CounterValue) by Computer | join kind=inner ( Heartbeat     | where OSType == `"Windows`" | summarize max(TimeGenerated) by Computer ) on Computer | project Computer, mtgPerf, AvailableMBytes=round(CounterValue), SampleCount; _linuxMem | union _windowsMem | where AvailableMBytes < _minAvailableMB | project Computer, TimeGenerated=mtgPerf, CounterValue=AvailableMBytes, AlertType_s = `"Low Available Memory`", Severity = 4, SeverityName_s = `"WARNING`", AffectedCI_s = strcat(Computer, `"/FreeMemoryMB/`"), AlertTitle_s = strcat(Computer, `": Low Available Memory`"), AlertDetails_s = strcat(`"Computer: `", Computer, `"\\r\\nAverage Free Memory: `", AvailableMBytes, `" MB\\r\\nSample Period: Last `", _timeWindow, `"\\r\\nSample Count: `", SampleCount, `"\\r\\nAlert Threshold: < `", _minAvailableMB, `" MB`")"
    ...
    Create-Alert `
        ...
        -FrequencyInMinutes 30 `
        -TimeWindowInMinutes 30 `
        -Query "let _timeWindow = 30m; Event | where TimeGenerated >= ago(_timeWindow) | where EventLog == `"System`" and Source == `"DISK`" or Source == `"Ntfs`" and EventID == 55 | join kind=inner ( Heartbeat | summarize max(TimeGenerated) by Computer ) on Computer | project Computer, TimeGenerated, AlertType_s = `"NTFS - File System Corrupt`", Severity = 4, SeverityName_s = `"WARNING`", AffectedCI_s = Computer, AlertTitle_s = strcat(Computer, `": NTFS - File System Corrupt`"), AlertDetails_s = strcat(`"Event Description:\\r\\n`", RenderedDescription)"
    ...
    Create-Alert `
        ...
        -Query "let _timeWindow = 24h; Event | where TimeGenerated >= ago(_timeWindow) | where EventLog == `"System`" and Source == `"DISK`" or Source == `"Ntfs`" and EventID == 55 | join kind=inner ( Heartbeat | summarize max(TimeGenerated) by Computer ) on Computer | project Computer, TimeGenerated, AlertType_s = `"NTFS - Unexpected Shutdown`", Severity = 4, SeverityName_s = `"WARNING`", AffectedCI_s = Computer, AlertTitle_s = strcat(Computer, `": NTFS - Unexpected Shutdown`"), AlertDetails_s = strcat(`"Event Description:\\r\\n`", RenderedDescription)"

    # Checking Backup Failure alert
    Create-Alert `
        -Subscription $sub `
        -AlertText "Backup Job Failed" `
        -AlertResourceGroupName $AlertResourceGroupName `
        -LogAnaWrkspc $LogAnaWrkspc `
        -ScheduledLogs ($LogAnaWrkspcLogVers.version -eq 2 -and $LogAnaWrkspcLogVers.scheduledQueryRulesEnabled -eq $true) `
        -ActionGroupId $actionGroupId `
        ...
        -FrequencyInMinutes 1440 `
        -TimeWindowInMinutes 1440 `
        -Query "let _timeWindow = 24h; AzureDiagnostics | where TimeGenerated >= ago(_timeWindow)  | where ResourceProvider == `"MICROSOFT.RECOVERYSERVICES`" and Category == `"AzureBackupReport`" | where OperationName == `"Job`" and JobOperation_s == `"Backup`" and JobStatus_s == `"Failed`" | project TimeGenerated, JobUniqueId_g, JobStartDateTime_s, JobOperation_s, JobStatus_s , JobFailureCode_s, JobDurationInSecs_s , AdHocOrScheduledJob_s"
    ...
    Create-Alert `
        ...
        -FrequencyInMinutes 1440 `
        -TimeWindowInMinutes 1440 `
        -Query "let _timeWindow = 24h; AzureDiagnostics | where TimeGenerated >= ago(_timeWindow) | where ResourceProvider == `"MICROSOFT.AUTOMATION`" and Category == `"JobLogs`" and (ResultType == `"Failed`" or ResultType == `"Suspended`") | summarize AggregatedValue = count() by RunbookName_s"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBzc2oFpOrooGs1
# L2yWqeo0DS6DL1wQdJzSZv35/59XXqCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIHPYDTQJnO/Hdit0
# pBcr8DnG6BqSuQDCw6TRXAGRH6BTMA0GCSqGSIb3DQEBAQUABIICALwWxanbXnow
# M0mjHFb8tqkrIf1SjH2zV/LJDYsnGrlDWniFQ78tlY2xq73BOOv11P+43NXcLKOc
# BT/P46Km/NVcLwXoP2dxYk2i8G5Ex22/Ls7a9SO5RlTDmvfHXbrXCbu9pyFw2yK4
# loUPXwbx6HRE7ocTc+Jv3FyE4/6vETqHF4JVeoL0MNGlrE9QUlboXCuU14il/rxe
# IZ2+xzUV3LMrAkiz5YTHIYVvnYjsIxHtEilpLyI3Rv42acXKGJD4JZkJwAWwGLCz
# j7DBNOeigiee2mQf82LkauqtpujbzDOh1lJn/mki6p7+VJgzztBgHAB66M5FYrde
# 4pOTmFz5qyWBi9agqotdpGKL43PesiAXAt/3oYGwckcdg3D6g29r+B6pqsS+yOWO
# 8JZYL1b5jENhLHb1iPxvy0i0tZNTjsjzgMQJt1yBSlRBzX6qmKvWFzMXZFbpNknw
# miXspJjhdJNh2947JBZZdBMLxnghrlu4yaN1hIoIBFlZlSr4s2v7HDqILfUzpPel
# RncKQqcWvGkY1UKVvKys/BtMsSWGRQcmMv0vSwLEEx+NNam0XbW9X7syDawaAE/p
# 908q7xTbS/Mvuy5ojUUhhPr5ABiMUA6q6OUK/6evC3BFYMlyMhNesgKbih5PrLDC
# MgxYVTNYdQu+jz8+h1b8kKmlbPsbWA7HoYIWuzCCFrcGCisGAQQBgjcDAwExghan
# MIIWowYJKoZIhvcNAQcCoIIWlDCCFpACAQMxDTALBglghkgBZQMEAgEwgd8GCyqG
# SIb3DQEJEAEEoIHPBIHMMIHJAgEBBgsrBgEEAaAyAgMBAjAxMA0GCWCGSAFlAwQC
# AQUABCBhs7nt9Z2Ga1rK3LoevtrKqr8N9spxJoYrDy4PracW2gIUMS+XuEpAqaGy
# 3Pr8+AZjtUXLqe4YDzIwMjYwMjA2MTE0ODMwWjADAgEBoFikVjBUMQswCQYDVQQG
...
# IEPH1BSoklTggHStUKpf2YDb0Bytt5W6+XK0YueeIQSJMIGwBgsqhkiG9w0BCRAC
# LzGBoDCBnTCBmjCBlwQgcl7yf0jhbmm5Y9hCaIxbygeojGkXBkLI/1ord69gXP0w
# czBfpF0wWzELMAkGA1UEBhMCQkUxGTAXBgNVBAoTEEdsb2JhbFNpZ24gbnYtc2Ex
# MTAvBgNVBAMTKEdsb2JhbFNpZ24gVGltZXN0YW1waW5nIENBIC0gU0hBMzg0IC0g
# RzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAMRqqU2Z0sJhG
# cBwtbd6XP3wMnoHrYFsPMoAyuVibU6hl1tRpmGArPalcVTXU7G72TJ1/M0Ro0tC4
# 5rO1g98RL+NoN0LxAq9y0wimAvGQpSS4TDHtf3SLJDm3rpkyoq63b+6UYz5kb6mW
# 9H5F1ifr3ZYDt7mVfKLyPDVsJ/qgw5v1N1ZCdOTrDGXe1rCTAabRA87T3B5nU/zo
# xvAjcvFK/0em8MK28woM1393Y32t8qMWN/CkE+nUk34wVexq486FKrMZXhOc9Cbm
# jwcaTIu325jF17di/76LNxZ8iju+zsg1NR8c2KQXIUNOd5St8O3NnVlH8TI00cEH
# MhpviRGs2gBfRbsCMRLEEFqVrciEFt4TQmiEPjz3nt4sgMU7Eq/+BDzWqn8cGzRa
# ibt3FS1P2S4hX38YOMjSHyXrs9PgfByvepVzcTWZVDee8/iiRGzsFg1RgNIH67JU
# Nz0id9/AT/0iRaOj7xexO+E63L+xEs/dhra3+SDJug1Ob9IoKqfs
