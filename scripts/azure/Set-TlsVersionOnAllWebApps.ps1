...
<#
.SYNOPSIS
Sets the minimum TLS version on all Azure Web Apps across one or more subscriptions.

.DESCRIPTION
The Set-TlsVersionOnAllWebApps.ps1 script updates the minimum TLS version setting for all Azure Web Apps within the configured subscriptions. It can target all subscriptions or a specific one, and optionally limit the update to a single web app by name. The script ensures the required Azure PowerShell modules are installed, logs into Azure, and records its activity in a transcript log file. It verifies whether each web app already has the desired TLS version and only modifies those that do not comply with the specified configuration.

.PARAMETER tlsVersion
Specifies the target TLS version to configure for the web apps. Default value is "TLS1_2".

.PARAMETER processOnlyWebAppWithName
Optional. If specified, the script only processes the web app with this name within the selected subscriptions.

.PARAMETER subscriptionName
Optional. Specifies a particular subscription name to process. If omitted, the script will iterate through all available subscriptions defined in the environment configuration.

.INPUTS
None. The script does not accept pipeline input.

.OUTPUTS
None. The script writes informational, warning, and error messages to the console and to a log file.

.EXAMPLE
PS> .\Set-TlsVersionOnAllWebApps.ps1 -tlsVersion "TLS1_2" -subscriptionName "ProductionSubscription"
...
Param(
    ...
    [string]$processOnlyWebAppWithName = $null,
...
Write-Host "Monitor | Set-TlsVersionOnAllWebApps | AZURE" -ForegroundColor $CommandInfo
...
foreach ($AlyaSubscriptionName in (([string]::IsNullOrEmpty($subscriptionName) ? $AlyaAllSubscriptions : @($subscriptionName)) | Select-Object -Unique))
{
    ...
    foreach ($WebApp in $WebApps)
    {
        ...
        if (-Not [string]::IsNullOrEmpty($processOnlyWebAppWithName) -and $processOnlyWebAppWithName -ne $WebAppName)
        {
            continue
        }

        Write-Host "Checking web app $WebAppName" -ForegroundColor $CommandInfo
        if ($WebApp.SiteConfig.MinTlsVersion -eq $tlsVersion)
        {
            Write-Host "  Set already to $tlsVersion"
        }
        else
        {
            Write-Warning "  Setting to $tlsVersion"
            $ret = Set-AzWebApp -ResourceGroupName $WebApp.ResourceGroup -Name $WebAppName -MinTlsVersion $tlsVersion
            if ($ret.SiteConfig.MinTlsVersion -eq $tlsVersion)
            {
                Write-Host "  Set successfully"
            }
            else
            {
                Write-Error "  ERROR: Setting to $tlsVersion failed!" -ErrorAction Continue
            }
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAfYreCWgeA7RhA
# Mm0nkdJ7UW5Vt1WBv7a6phwkk6sAP6CCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIF1myATS
# wk/psGbevyVNObLWoCkKn3eOshCDezDYgYQlMA0GCSqGSIb3DQEBAQUABIICAKxz
# W7uOd+MwVR7rVpuLrCsqBxdiXlCrxKFspbYdYPEkA5WoVdiV9FIULCZJFo9aZ96g
# 5VrhhiZe38cqOXgH1QPTRGdGfe7PqtVOV40r+c0Xk1YKShzC2eF5nodaqhX9UcO9
# MJIsPZiTW6kUH40txZaVSrJBXKdB3Cg6tVRDMFtjmg8frj9trEWv/9x6fmV+XrBV
# iHXMqOHSZYEJ4bVpPyxpELhe2SFOcWj8Re7xtygHeCY+pSTmPP/b6d0K1DbnTNJ3
# oSx9fDgMAF4npgnxiNLoQsxEX6s4uUuLPHM1VHxyZGmNBfTXDqm/w3leqtv3/WPG
# JDA94luJbp26DfKW/IRyx9ATgUFvKSKTSYhqFwpIofgX6UMKAZShJ6AD4xqv71CL
# rrOHUM6/Jc0xWbuUgb/9dhzQuasBMk91+Tv6Yd88KCNnrtTm3sBNXy3WZ3QU9yh5
# S9s3I3EgKX5iXOCOMZ9mSmQ4q1dToCW2roM7d57p+xvZrU7b6XecXW5mYDrREHBj
# SmfBve1ZXpcmbvcBKUlCd3YZh5J+bTqrR9SOpWbsoxieg7r6c/3OPaoO/EnUPLIP
# NfVrO8XFU8JXPk/KiObKbOOCtk1xUYdUJiDG1rykbvYv3DkgXZmabLi6to8YdwA+
# wdb09frr8b7P/hhto03GoHyp8X2jo/CzpuJFvU0XoYId7TCCHekGCisGAQQBgjcD
# AwExgh3ZMIId1QYJKoZIhvcNAQcCoIIdxjCCHcICAQMxDTALBglghkgBZQMEAgIw
# geQGCyqGSIb3DQEJEAEEoIHUBIHRMIHOAgEBBgsrBgEEAaAyAgMCAjAxMA0GCWCG
# SAFlAwQCAQUABCDFkiJfYgzvHf2dM9TjECz3lymA2yf0iWeOY5aMyJJ7cgIUMHy+
# DPtCq2uaDbSk/JuuFwztVl0YDzIwMjYwNTIxMDY1OTIwWjADAgEBoF2kWzBZMQsw
...
# BDEyBDBt71upv62d3kV2jp5lxpSnLaI+0SBtB1bodkjf2YUKu1jTYXuWipKpyVay
# OHun8QcwgbQGCyqGSIb3DQEJEAIvMYGkMIGhMIGeMIGbBCCDKtcuUj/erIP6RpS8
# 58bMJhdkiChmVmWIyK3KOoOFUTB3MGKkYDBeMQswCQYDVQQGEwJCRTEZMBcGA1UE
# ChMQR2xvYmFsU2lnbiBudi1zYTE0MDIGA1UEAxMrR2xvYmFsU2lnbiBPZmZsaW5l
# IFI0NSBUaW1lc3RhbXBpbmcgQ0EgMjAyNQIRAIRyP8GVzBbx2yui9mDfK+QwDQYJ
# KoZIhvcNAQEMBQAEggGAsUF9ciFhUDUEZrDqmrqx+i/VZSfkMEd13Bi+ms49/qY4
# 4nY0ismTVP36ag9qPwIOV6JiCv/XWbsWG07Ms1LGXeObd12EO6cbE+Axob1sB76v
# j9v/nJe9vZ2tgHS8BaTvUW2eC4YZBeOR342UGtv+Xy21xOFvr3Sz8YJhd8XitBd4
# uIUklxk1ZivI4kZg9bs6T0GqXnayOBI62U9m90JiT9T0dP9Id9B6tkx/h9Lh0vpU
# rhq1G8CE8J/P7YbpxCkPzWz5UKLmwHMoaq0sAcr0naQSToPXqICyVUCycoFFR10Y
# 2MCUKUGtDLnvUWMywZ2QxikB5T+qXcPralrfqJBAwfI2pXAdOLKrqdGZTwQMVP0m
# X+Y1sZVM7QZBChweZlDs3lFmWGe4zb67DLQEd4li1V2ByxFvRswRU1XxOX3vu7iz
# nHmPr8yPZg9QfvMRLfsAvYw35/wCN+9km5E1Z2yGRPKqNDAQEqxG7ghCjwhzsvPr
# ui70zBJgeO/8rWAVP07E
