...
<#
    ...
    15.05.2026 Konrad Brunner       User icon update

#>

<#
.SYNOPSIS
Creates and configures a "DoNotReply" shared mailbox in Exchange Online and disables its login in Azure Active Directory.

.DESCRIPTION
The Setup-DoNotReplySharedMailbox.ps1 script automates the setup of a shared "DoNotReply" mailbox used for outgoing system notifications. It ensures required modules are installed, connects to Exchange Online to create and configure the mailbox if it does not exist, applies regional and calendar configurations, and blocks the mailbox account from signing in within Azure Active Directory. The script logs all actions and errors for traceability.

.INPUTS
None. The script does not take any input objects.

.OUTPUTS
Creates or updates a "DoNotReply" shared mailbox in Exchange Online and disables its Azure AD login. Outputs logging information to a transcript file.

.EXAMPLE
PS> .\Setup-DoNotReplySharedMailbox.ps1
...
try {
    ...
    $DoNotReplyEmail = "DoNotReply@$AlyaDomainName"
    $mailbox = Get-Mailbox -Identity $DoNotReplyEmail -ErrorAction SilentlyContinue
    if (-Not $mailbox) {
        Write-Warning "Creating the shared mailbox DoNotReply"
        New-Mailbox -Shared -Name "DoNotReply" -DisplayName "$AlyaCompanyName DoNotReply" -Alias "DoNotReply" -PrimarySmtpAddress $DoNotReplyEmail
    }
    ...
    do {
        try {
            ...
            $mailbox = Get-Mailbox -Identity $DoNotReplyEmail

            $doNotReplyUserId = $mailbox.ExternalDirectoryObjectId
        
            Set-MailboxRegionalConfiguration -Identity $DoNotReplyEmail -TimeZone $AlyaTimeZone
            Set-MailboxCalendarConfiguration -Identity $DoNotReplyEmail -WorkingHoursTimeZone $AlyaTimeZone
            break
        }
        catch {
...
if ($doNotReplyUserId) {
    ...
    Write-Warning "DoNotReply user account not found in Azure AD. Skipping account disablement."
}
if ($doNotReplyUserId) {
    Write-Host "  Setting account icon"
    & "$AlyaScripts\aad\Set-ProfilePicsOnAllUsers.ps1" -SetOnlyOnUpn $DoNotReplyEmail
}
else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCASqfGisGGrFwTB
# eorepE05StsJgeiitbVWB1maO9DLK6CCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIHJha9qi
# 2VRF+TNCNBFyHTJVlk9MUNp6o1z/6jQ3JPbPMA0GCSqGSIb3DQEBAQUABIICAI4/
# /R0hhLLyj7/6cJ+OxrWqgohlq6WeckZEXGGcgT1C7Ci0iVaW6GuDNS9hP1rvmxHn
# 2bKg57FKzJhgDvQQdmBejC/+OE2La4ArFqZqotPDjF8pGPLL/UDB1Ptor8mbZD1j
# F8hI95gggCiSIvow0Q/lmxHLfFVfMiSBAcmFhEKAYoPhD5Mbm8/wUMzrpudM55Kv
# Y4wFxIBE2y9RWmvEY3MfRHXQGpT+4hGYgUOG6gphGoVVhubaSGMT0Z6RUYoyoK9c
# X97lEYvqHKjGN9UdL+H1irf+UhNvWpqeQVkOvTDWRD5GwL9UMDw0Znyw2B38Wk6k
