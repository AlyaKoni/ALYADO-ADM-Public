...
<#
    ...
    ---------- -------------------- ----------------------------
    15.07.2025 Konrad Brunner       Initial Creation
    06.02.2026 Konrad Brunner       Added powershell documentation
    15.05.2026 Konrad Brunner       Login name and mail the same

#>

<#
.SYNOPSIS
Creates and configures a shared mailbox named "Intune" in Exchange Online and optionally disables its corresponding Azure AD user account.

.DESCRIPTION
The Setup-IntuneSharedMailbox.ps1 script ensures that the required shared mailbox for Intune integration exists within Exchange Online. It connects to Exchange Online, creates the shared mailbox if it does not exist, applies regional and time zone settings, and optionally disables the associated user account in Azure AD. The script also checks for and installs required PowerShell modules, logs all output, and handles error and retry scenarios gracefully.

.INPUTS
None. The script does not accept any pipeline input.

.OUTPUTS
None. The script writes progress and error information to the console and to a transcript log.

.EXAMPLE
PS> .\Setup-IntuneSharedMailbox.ps1
...
Start-Transcript -Path "$($AlyaLogs)\scripts\exchange\Setup-IntuneSharedMailbox-$($AlyaTimeString).log" | Out-Null
...
Write-Host "INTUNE | Setup-IntuneSharedMailbox | EXCHANGE" -ForegroundColor $CommandInfo
Write-Host "=====================================================`n" -ForegroundColor $CommandInfo

Write-Host "Configuring Intune Shared Mailbox in Exchange"
try
{
    ...
    $IntuneEmail = "Intune@$AlyaTenantName"
    $mailbox = Get-Mailbox -Identity $IntuneEmail -ErrorAction SilentlyContinue
    if (-not $mailbox)
    {
    ...
    do
    {
        try
        {
            ...
            $mailbox = Get-Mailbox -Identity $IntuneEmail
...
Write-Host "`n`n=====================================================" -ForegroundColor $CommandInfo
Write-Host "EXCHANGE | Setup-IntuneSharedMailbox | Azure" -ForegroundColor $CommandInfo
Write-Host "=====================================================`n" -ForegroundColor $CommandInfo

if ($intuneUserId)
{
    Write-Host "  Blocking Intune account from login"
    LoginTo-Az -SubscriptionName $AlyaSubscriptionName
    Update-AzADUser -ObjectId $IntuneUserId -EnableAccount $false
}
else
{
    Write-Warning "Intune user account not found in Azure AD. Skipping account disablement."
}
if ($intuneUserId)
{
    Write-Host "  Setting account icon"
    & "$AlyaScripts\aad\Set-ProfilePicsOnAllUsers.ps1" -SetOnlyOnUpn $IntuneEmail
}
else
{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAR7lyBdz7o5QUB
# 8/9qE8euDoaBBSC3OiCxsy4aLYKhB6CCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIEIsj/Td
# JwJavMKaUwmWZErGhUPqA2uMCjilAxuah6ctMA0GCSqGSIb3DQEBAQUABIICADBp
# ilZeQzPcepfmIkBfSSfAUhgDa+xBTMjmiS5JYmwymTEy5j9JlQqKoqu225nWS3Ba
# pe364QShy8MmNONv/FLHyUk1d4oHMu/0I+fEPs3HvKF/GjF/LZPNGuCQLhr7Evwj
# OmddcXYnkM2HCNzRQ+6iNeNUtRwmx0z4s/rveUcW9606Cf0S4tqNqQSxBZ3VDlrj
# mWbSihPOux/CB7RZ+4jLJOdVqmHjTC6QOx3Mec/boD2+Yl+iWujmEAoyyguQVFU+
