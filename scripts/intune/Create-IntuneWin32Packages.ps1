...
<#
.SYNOPSIS
Creates and updates Intune Win32 application packages based on predefined folder structures and configuration scripts.

.DESCRIPTION
The Create-IntuneWin32Packages.ps1 script automates the process of preparing, downloading, organizing, and packaging software installers into Intune Win32 packages (.intunewin). It scans specified application directories, cleans or recreates necessary folders, downloads installation files, prepares content archives, and invokes the Microsoft Intune Win32 Packaging Tool to generate deployment-ready packages. The script supports selective app processing, continuation from a specific point, and reuse of existing packages.

.PARAMETER CreateOnlyAppWithName
Specifies the name of an application folder to process exclusively. If provided, only that app will be packaged.

.PARAMETER ContinueAtAppWithName
Allows script execution to resume at a specific app name, skipping all preceding applications.

.PARAMETER AppsPath
Defines the subdirectory under the Intune data folder where application directories are located. Defaults to "Win32Apps".

.PARAMETER ReuseExistingPackages
Indicates whether to reuse existing .intunewin packages if found. Defaults to $false, which triggers package recreation.

.INPUTS
None. The script processes data from the specified Intune application data directories.

.OUTPUTS
Creates Intune Win32 application packages (.intunewin) and logs transcript files under the configured log directory.

.EXAMPLE
PS> .\Create-IntuneWin32Packages.ps1 -AppsPath "CustomApps" -ReuseExistingPackages $true
...
foreach($packageDir in $packageDirs)
{
    ...
    if (-Not $intunewinFile -or $ReuseExistingPackages -eq $false)
    {
        ...
        $hasLongPathNames = Test-Path (Join-Path $packageDir.FullName "HasTooLongPathNames.txt")
        if ($hasLongPathNames)
        {
            $contentPathOrig = $contentPath
            $contentPath = "C:\Temp\IntuneWinAppPackage"
            New-Item -Path $contentPath -ItemType Directory -Force
            robocopy /mir /XJ "$contentPathOrig" "$contentPath"
        }
        ...
        {
            $fnd = $toInstall[0]
            $setupPreference = Get-Item -Path (Join-Path $packageDir.FullName "SetupPreference.txt") -ErrorAction SilentlyContinue
            if ($setupPreference)
            {
                [regex]$setupPreference = $setupPreference | Get-Content
                foreach($setup in $toInstall)
                {
                    if ([regex]::Match($setup.Name, $setupPreference, [Text.RegularExpressions.RegexOptions]'IgnoreCase, CultureInvariant').Value)
                    {
                        $fnd = $setup
                        break
                    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBn3aSbPUuQz5pY
# /v7Uk1FePFEsWlCVYznaWnZpvpYE1KCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE