...
<#
.SYNOPSIS
Configures an Azure VPN Gateway, generates necessary certificates, and stores configuration data and certificates in Azure Key Vault.

.DESCRIPTION
The Configure-VPNGateway.ps1 script automates the provisioning and configuration of an Azure VPN Gateway. It validates necessary Azure resources such as the resource group, virtual network, subnet, public IP, and Key Vault. The script creates the VPN Gateway if not already present, generates a root certificate and multiple client certificates for VPN authentication, and uploads them to Azure Key Vault. Additionally, it retrieves and secures the VPN client configuration profile for deployment. Log files are generated to track execution.

.INPUTS
None. This script does not accept pipeline input.

.OUTPUTS
Creates or updates Azure resources including public IPs, virtual network gateways, and certificates in Azure Key Vault. Generates log files in the Alya logs directory.

.EXAMPLE
PS> .\Configure-VPNGateway.ps1
...
if (-Not $gwip)
{
    ...
    $gwip = New-AzPublicIpAddress -ResourceGroupName $ResourceGroupName -Name $VirtualNetworkGatewayIpName -Location $AlyaLocation -Sku "Standard" -AllocationMethod "Static" -Zone @("1", "2", "3") -IpAddressVersion "IPv4" -DomainNameLabel $VirtualNetworkGatewayIpName -Tag @{displayName="VPN Gateway Public IP";ownerEmail=$Context.Account.Id}
}
else
{
    Write-Host "Updating"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCXdISLJGDobJ4N
# CPSV46DGcYslYo2/+WBc07tX+5vL5aCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIESyF0X/w9iTzuQv
# B6CThs7rh1d32H9Wlnq/1FuVOsM8MA0GCSqGSIb3DQEBAQUABIICAG2/f5PZNOjH
# UH5jqO+ss3AVX6qBa9Aw3VXZqFuCV2Ze5wrxWvh8jLh07U89E18gfJlr8zl/dJ3k
# Xt4T7aJgT86YcloKX61o/EEX7ID9DoQzQ2PpdybmTHWFFTjh1Vssj0bKKZBwyzn2
# mPcRLbOVuUiEExaS4uoVt4Lf1yyIp2olAitXcGkxjZ1OR19+3PaO48UnSYEg04hU
# j2CEYNS/Ln/Tu02jHqujcde/W4oXmK9QYFZcv8XShPDii88SW3PAJlqSB7r9dJgV
# Dya12ZbKzXB6C5MnLoxef+3TmMfvcAcAya0vTdoDOtEXupfj3K7uHfx90BGuKUIu
# RunN9GGBBRAJ7xr2zwTkFW1ARe/H50SGwMoT7YLd0rZq23xnmz7DGpPIL9ulUwjK
# nWzdS5XP/5eH9QjnqgxzcopmHnlu9Qxqjcs7oT/SXd3B0qrgzAiKUIQFFfG3M5o3
# uRNO1eK3qbSqVdhovuK7hc90qX808YZKcn4L03LVOFdFFb8HoUGpnzfoTT0QUytO
# /vBz/kmQMiXJZZMMw0bsPPENLWLAQH5hqDLdFUnokNgdCyaym7sNPjFdTZwCW+bg
# a62iwzfogo/nWZpwyvvMYobXEjbQRH/Jlrb1COfC+hCck65Lzx3yN+ZlFOVU4NuV
# MXPT0Ay8DCR4zw346wwG8obQEKLlmI1ooYId7TCCHekGCisGAQQBgjcDAwExgh3Z
...
# AQUABCCNgZTRbnkszmUmV0+VyiVkvRvIq6sNXjpEJjy9k7XkJgIUf/u8Q7HzyBn3
# jyr0fi8geOnQvIEYDzIwMjYwNTA0MDcyNzE3WjADAgEBoF2kWzBZMQswCQYDVQQG
...
# MBwwCwYJYIZIAWUDBAICoQ0GCSqGSIb3DQEBDAUAMD8GCSqGSIb3DQEJBDEyBDBJ
# 32fjS+4UPgYr9tVvO2n+S1auKbwZaOlgIKa5GfhbZcCm4CCegaHnsHGVDfUeeHgw
...
# AQEMBQAEggGAfr8/rZZ9xxYTQj5wjQUFahPp/zJA3XSN9rGnJNSKK53U63CPDWt6
# EJZf8svpLLa+KriR1cQPHjyX3kAhu6O04YOmU9PniyPve/FtgNF5sPtdl/JHGhqq
# 24ZkxcMKTfgRVuy5yOHdwErm3bKUeoUasL0BKBMM1TqVR5QtVJd3e4iUUe0OF/E0
# UKjwIf5yjbW304L28Z0mmVAoHjXgaODoxKLm+OesZVTikZR2JbMtGuBIHzSe+xgF
# rcyhZVza4RUD5AzuL7+NcVy7PglIzoeCzbz3Ryf7t3sIAvyj6aaajexV50qORV8U
# pPEbRIKerDpBKDYjqSMBIaYOaxPbCIX6rxKPgUfNDh27AvupoDLtoPHOSfi0VAfE
# Hmcwqi9bscLYvpL0rmEK6i4mCEWm+W8QOx2UrxSihY9SID0eiVcI1GxweMEKiPtB
# 0pOaEXGn8Hss/1F4lJ0FPkH/t9+1QYgvbdWbvaVAVLlzaL2XVGlOK+cvFn/WHjOM
# D2jroizTTZNw
