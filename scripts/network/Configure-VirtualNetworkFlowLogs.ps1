...
.EXAMPLE
PS> .\Configure-VirtualNetworkFlowLogs.ps1
...
if (-not $UserAssignedIdentityName) { $UserAssignedIdentityName = "$($AlyaNamingPrefix)uami$($AlyaResIdMainNetwork)" }
...
# Checking existing watchers
Write-Host "Checking existing watchers" -ForegroundColor $CommandInfo
...
foreach($sub in $subs)
{
    ...
    {
        if (-Not ($locations -contains $existing.Location))
        {
...
foreach($loc in $locations)
{
    ...
    $StorageAccountNameLoc = "$($StorageAccountName)$locShort"

    # Checking storage account
    Write-Host "Checking storage account $StorageAccountNameLoc" -ForegroundColor $CommandInfo
    $StrgAccount = Get-AzStorageAccount -ResourceGroupName $ResourceGroupName -Name $StorageAccountNameLoc -ErrorAction SilentlyContinue
    if (-Not $StrgAccount)
    {
        Write-Warning "Storage account not found. Creating the storage account."
        $StrgAccount = New-AzStorageAccount -Name $StorageAccountNameLoc -ResourceGroupName $ResourceGroupName -Location $loc -SkuName "Standard_LRS" -Kind StorageV2 -MinimumTlsVersion "TLS1_2" -AccessTier Cool -Tag @{displayName="NetworkDiagnostic Log Storage";ownerEmail=$Context.Account.Id}
        if (-Not $StrgAccount)
        {
            Write-Error "Storage account creation failed. Please fix and start over again" -ErrorAction Continue
            Exit 1
        }
    }
...
Write-Host "Checking user assigned managed identity $UserAssignedIdentityName" -ForegroundColor $CommandInfo
...
{
    Write-Warning "User assigned managed identity not found. Creating the user assigned managed identity."
    $UserAssignedIdentity = New-AzUserAssignedIdentity -Name $UserAssignedIdentityName -ResourceGroupName $ResourceGroupName -Location $AlyaLocation
    if (-Not $UserAssignedIdentity)
    {
        Write-Error "User assigned managed identity creation failed. Please fix and start over again" -ErrorAction Continue
        Exit 1
    }
}
...
foreach($loc in $locations)
{
    $loc = $loc.Replace("switzerland", "ch").Replace("germany", "de").Replace("france", "fr").Replace("europe", "eu").Replace("austria", "at").Replace("norway", "no").Replace("belgium", "be").Replace("italy", "it").Replace("spain", "es")
    $StorageAccountNameLoc = "$($StorageAccountName)$loc"
    
    # Assigning role to storage account for user assigned managed identity
    Write-Host "Checking role assignment for user assigned managed identity $UserAssignedIdentityName on storage account $StorageAccountNameLoc" -ForegroundColor $CommandInfo
    $StrgAccount = Get-AzStorageAccount -ResourceGroupName $ResourceGroupName -Name $StorageAccountNameLoc -ErrorAction SilentlyContinue
    $RoleAssignment = Get-AzRoleAssignment -RoleDefinitionName "Storage Blob Data Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $StrgAccount.Id -ErrorAction SilentlyContinue | Where-Object { $_.Scope -eq $StrgAccount.Id }
    $Retries = 0;
    While ($null -eq $RoleAssignment -and $Retries -le 6)
    {
        $RoleAssignment = New-AzRoleAssignment -RoleDefinitionName "Storage Blob Data Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $StrgAccount.Id -ErrorAction SilentlyContinue
    ...
    if ($Retries -gt 6)
    {
        throw "Was not able to set role assigment Storage Blob Data Contributor for user assigned identity $($UserAssignedIdentity.PrincipalId) on scope $($StrgAccount.Id)"
    }
...
if ($WithTrafficAnalytics)
{
    ...
    # Assigning role to log analytics workspace for user assigned managed identity
    Write-Host "Checking role assignment for user assigned managed identity $UserAssignedIdentityName on log analytics workspace $LogAnaWrkspcName" -ForegroundColor $CommandInfo
    $RoleAssignment = Get-AzRoleAssignment -RoleDefinitionName "Log Analytics Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $LogAnaWrkspc.ResourceId -ErrorAction SilentlyContinue | Where-Object { $_.Scope -eq $LogAnaWrkspc.ResourceId }
    $Retries = 0;
    While ($null -eq $RoleAssignment -and $Retries -le 6)
    {
        $RoleAssignment = New-AzRoleAssignment -RoleDefinitionName "Log Analytics Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $LogAnaWrkspc.ResourceId -ErrorAction SilentlyContinue
        Start-Sleep -Seconds 10
        $RoleAssignment = Get-AzRoleAssignment -RoleDefinitionName "Log Analytics Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $LogAnaWrkspc.ResourceId -ErrorAction SilentlyContinue | Where-Object { $_.Scope -eq $LogAnaWrkspc.ResourceId }
        $Retries++;
    }
    if ($Retries -gt 6)
    {
        throw "Was not able to set role assigment Log Analytics Contributor for user assigned identity $($UserAssignedIdentity.PrincipalId) on scope $($LogAnaWrkspc.ResourceId)"
    }
    $RoleAssignment = Get-AzRoleAssignment -RoleDefinitionName "Monitoring Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $LogAnaWrkspc.ResourceId -ErrorAction SilentlyContinue | Where-Object { $_.Scope -eq $LogAnaWrkspc.ResourceId }
    $Retries = 0;
    While ($null -eq $RoleAssignment -and $Retries -le 6)
    {
        $RoleAssignment = New-AzRoleAssignment -RoleDefinitionName "Monitoring Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $LogAnaWrkspc.ResourceId -ErrorAction SilentlyContinue
        Start-Sleep -Seconds 10
        $RoleAssignment = Get-AzRoleAssignment -RoleDefinitionName "Monitoring Contributor" -PrincipalId $UserAssignedIdentity.PrincipalId -Scope $LogAnaWrkspc.ResourceId -ErrorAction SilentlyContinue | Where-Object { $_.Scope -eq $LogAnaWrkspc.ResourceId }
        $Retries++;
    }
    if ($Retries -gt 6)
    {
        throw "Was not able to set role assigment Monitoring Contributor for user assigned identity $($UserAssignedIdentity.PrincipalId) on scope $($LogAnaWrkspc.ResourceId)"
    }

}

function Check-WatcherFlowLog
{
    param (
        [object]$Watcher,
        [object]$StrgAccount,
    ...
    Write-Host "  Checking flow log $FlowLogName"
    $flowLogs = Get-AzNetworkWatcherFlowLog -NetworkWatcher $Watcher -ErrorAction SilentlyContinue
    $flowLog = $flowLogs | Where-Object { $_.Name -eq $FlowLogName }
    if ($flowLog)
    {
        Write-Host "    Flow log $FlowLogName already exists. Checking configuration."
        $dirty = $false
        $enabled = $flowLog.Enabled
        if ($flowLog.StorageId -ne $StrgAccount.Id)
        {
            Write-Warning "Flow log $FlowLogName is configured with a wrong storage account. Fixing now."
            $dirty = $true
        }
        if ($flowLog.StorageId -ne $StrgAccount.Id)
        {
        ...
        if (($flowLog.FormatText | ConvertFrom-Json).Version -ne 2)
        {
            Write-Warning "Flow log $FlowLogName is configured with a wrong format version. Fixing now."
            $dirty = $true
        }
        if ($flowLog.Location -ne $VNet.Location)
        {
            Write-Warning "Flow log $FlowLogName is configured with a wrong location. Fixing now."
            $dirty = $true
        }
        if ($flowLog.TargetResourceId -ne $ResourceId)
        {
            Write-Warning "Flow log $FlowLogName is configured with a wrong target resource. Fixing now."
            $dirty = $true
        }
        if ($flowLog.RetentionPolicy.Enabled -ne $true)
        {
            Write-Warning "Flow log $FlowLogName is configured with a wrong retention policy. Fixing now."
            $dirty = $true
        }
        if ($flowLog.RetentionPolicy.Days -ne $RetentionInDays)
        {
            Write-Warning "Flow log $FlowLogName is configured with a wrong retention policy. Fixing now."
            $dirty = $true
        }
        if ($flowLog.Identity.UserAssignedIdentities.Keys -ne $UserAssignedIdentity.Id)
        {
            Write-Warning "Flow log $FlowLogName is configured with a wrong user assigned identity. Fixing now."
            $dirty = $true
        }
        # if ($flowLog.FlowAnalyticsConfiguration -ne $LogAnaWrkspc.ResourceId)
        # {
        #     Write-Warning "Flow log $FlowLogName is configured with a wrong flow analytics configuration. Fixing now."
        #     $dirty = $true
        # }
        if ($dirty)
        {
            $FlowLog = Set-AzNetworkWatcherFlowLog -NetworkWatcher $watcher -Name $FlowLogName `
                -Enabled $enabled -TargetResourceId $ResourceId -StorageId $StrgAccount.Id `
                -EnableRetention $true -RetentionPolicyDays $RetentionInDays -FormatVersion 2 `
                -UserAssignedIdentityId $UserAssignedIdentity.Id -Force 
    ...
    {
        Write-Warning "  Flow log not found. Creating it now."
        $res = Get-AzResource -ResourceId $ResourceId
        $FlowLog = New-AzNetworkWatcherFlowLog -NetworkWatcher $watcher -Name $FlowLogName `
            -Enabled $false -TargetResourceId $ResourceId -StorageId $StrgAccount.Id `
            -EnableRetention $true -RetentionPolicyDays $RetentionInDays -FormatVersion 2 `
            -UserAssignedIdentityId $UserAssignedIdentity.Id 
            <#-EnableTrafficAnalytics:($WithTrafficAnalytics ? $true : $false) -TrafficAnalyticsWorkspaceId ($WithTrafficAnalytics ? $LogAnaWrkspc.ResourceId : $null) -TrafficAnalyticsInterval $TrafficAnalyticsInterval#>
        $flowLog = Get-AzNetworkWatcherFlowLog -NetworkWatcher $Watcher -Name $FlowLogName
        if (-Not $FlowLog)
        {
            Write-Error "Flow log $FlowLogName creation failed. Please fix and start over again" -ErrorAction Continue
        }
        else
        {
            Write-Host "  Flow log created."
        }
    }
}
...
foreach($sub in $subs)
{
    ...
    foreach($VNet in $VNets)
    {
        ...
        if (-Not $watcher)
        {
            Write-Warning "Network watcher $NetworkWatcherName not found. Please create it first."
            continue
        }
        ...
        Check-WatcherFlowLog -Watcher $watcher -StrgAccount $StrgAccount -FlowLogName $FlowLogName -ResourceId $VNet.Id

        # Processing subnets
        foreach($SNet in $VNet.Subnets)
        {
            Write-Host "Processing subnet $($SNet.Name)" -ForegroundColor $CommandInfo
