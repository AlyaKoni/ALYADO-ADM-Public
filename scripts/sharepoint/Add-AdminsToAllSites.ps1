...
<#
.SYNOPSIS
Adds specified administrator users and groups as site collection administrators to all SharePoint Online sites in the tenant.

.DESCRIPTION
The Add-AdminsToAllSites.ps1 script connects to the SharePoint Online tenant using PnP PowerShell, retrieves all site collections, and ensures that a defined set of users and groups are added as site collection administrators. The script determines existing administrative roles such as Global Administrator and SharePoint Administrator and combines them with additional provided user or group inputs. It updates each site collection so that required administrators are consistently applied across the tenant.

.PARAMETER adminGroups
An optional array of group names or identifiers to be added as site collection administrators to every site.

.PARAMETER adminUsers
An optional array of individual user names or identifiers to be added as site collection administrators to every site.

.INPUTS
None. The script does not accept pipeline input.

.OUTPUTS
None. The script writes progress and result information to the host and a transcript log file.

.EXAMPLE
PS> .\Add-AdminsToAllSites.ps1 -adminGroups "SP_Admins" -adminUsers "user1@contoso.com","user2@contoso.com"
...
Write-Host "SharePoint | Add-AdminsToAllSites | O365" -ForegroundColor $CommandInfo
...
foreach ($site in $sitesToProcess)
{
    ...
    if ($tsite.OwnerLoginName -ne $sprimaryAdmin)
    {
        try {
            ...
            $actualUser = (Get-PnPProperty -Connection $adminCon -ClientObject (Get-PnPWeb -Connection $adminCon) -Property CurrentUser).LoginName
            try {
                Set-PnPTenantSite -Connection $adminCon -Identity $site.Url -PrimarySiteCollectionAdmin $actualUser
            }
            catch {
                try {
                    Add-PnPSiteCollectionAdmin -Connection $siteCon -PrimarySiteCollectionAdmin $actualUser
                }
                catch {
                    try {
                        ...
                        Write-Warning "Not able to add primary admin $sprimaryAdmin or $actualUser"
                        Write-Warning "Please add $actualUser as site collection admin or use the global admin role and rerun this script"
                        continue
                    }
                }
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDeHR/oGhLNy+1z
# LRPY4fCTdMk75TfYYCelI7erXJ+KDKCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIAUsd0KASAwYNKWd
# 5vssQGWdHFFRFcPrNIlxbPUS3t6nMA0GCSqGSIb3DQEBAQUABIICAJMInd3quB8Q
# MTs7C6Iv6s/ejtqQT08rWSuWRZY6y/ggrVNS6P7y9dDoeVsgMRZwXagte+Ja1BVD
# TlBvGx3TF/LlpMUH+Nf16yn2IlyMN3OirkNkEIx70FeaBwzxYTRoPHNXsrNml8mx
# Y74MEoFvWJ1psAyKK/R/418aB5G4lQbMTdmCgXiLe9gDaNk8HiWfVOplG6tuNrBR
# 74SSWxZytbrkhras667dtWEeAfF5EbElAQEYFN/ZjuBXjPDOwzwTJ8sAE8FUnduV
