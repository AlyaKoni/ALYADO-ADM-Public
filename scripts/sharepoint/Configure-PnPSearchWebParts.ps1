...
<#
.SYNOPSIS
Configures and deploys the PnP Modern Search Web Parts package to a SharePoint Online tenant.

.DESCRIPTION
The Configure-PnPSearchWebParts.ps1 script automates the process of installing and deploying the PnP Modern Search Web Parts package to a SharePoint Online tenant. It ensures that required modules are installed, connects to the appropriate SharePoint Admin Center, checks and retrieves the App Catalog URL, downloads the latest release of the PnP Modern Search Web Parts package from GitHub, uploads and deploys it to the tenant app catalog, and prompts for necessary permission approval. The script supports multi-geo environments and can optionally use a Selenium browser instance for automation tasks.

.PARAMETER seleniumBrowser
Optional Selenium browser object to be used for automated browser actions. If not specified, a new browser instance is created or a global one is reused.

.PARAMETER multiGeoAdminUrl
Optional URL for the SharePoint Admin Center in a multi-geo tenant environment. If not provided, the script uses the default admin URL defined in the environment configuration.

.INPUTS
None. The script does not accept pipeline input.

.OUTPUTS
None. The script performs deployment actions and logs the process but does not return output objects.

.EXAMPLE
PS> .\Configure-PnPSearchWebParts.ps1 -multiGeoAdminUrl "https://contoso-admin.sharepoint.com"
...
Write-Host "SharePoint | Configure-PnPSearchWebParts | O365" -ForegroundColor $CommandInfo
...
if (-Not (Test-Path "$AlyaTemp\PnPSearchWebParts"))
{
    New-Item -Path "$AlyaTemp\PnPSearchWebParts" -ItemType Directory -Force | Out-Null
}

$nextUri = "https://github.com/microsoft-search/pnp-modern-search/releases/latest"
do {
    try {
        $req = Invoke-WebRequestIndep -Uri $nextUri -MaximumRedirection 0 -UseBasicParsing -ErrorAction Ignore
    }
    catch {
        $req = $_.Exception.Response
    }
    $nextUri = $req.Headers.Location
    if ($nextUri) { $contentUrl = $nextUri }
} while ($nextUri)
$actVersion = $contentUrl.AbsoluteUri.Substring($contentUrl.AbsoluteUri.LastIndexOf("/") + 1)
$req = Invoke-WebRequestIndep -UseBasicParsing -Uri "https://github.com/microsoft-search/pnp-modern-search/releases/expanded_assets/$actVersion"
[regex]$regexAzp = "[^`"]*pnp-modern-search/releases/download[^`"]*\.sppkg"
$newUrl = [regex]::Match($req.Content, $regexAzp, [Text.RegularExpressions.RegexOptions]'IgnoreCase, CultureInvariant').Value
$downloadUrl = "https://github.com" + $newUrl
Invoke-WebRequestIndep -UseBasicParsing -Uri $downloadUrl -Method GET -OutFile "$AlyaTemp\PnPSearchWebParts\pnp-modern-search-parts-v4.sppkg"
...
$app = Add-PnPApp -Connection $appSiteCon -Path "$AlyaTemp\PnPSearchWebParts\pnp-modern-search-parts-v4.sppkg" -Scope "Tenant" -Overwrite -Publish
$appId = $app.Id
$retries = 20
$app = $null
do
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCkZlD96vTho00u
# jZhkvbKvp+cuoIbke6LteMpPY78Q5KCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEI