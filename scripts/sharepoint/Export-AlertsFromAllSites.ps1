...
<#
    ...
    ---------- -------------------- ----------------------------
    21.05.2026 Konrad Brunner       Initial Version

#>

<#
.SYNOPSIS
Exports a report of all alerts across SharePoint Online site collections, including detailed information.

.DESCRIPTION
The Export-AlertsFromAllSites.ps1 script connects to a SharePoint Online tenant using PnP PowerShell and enumerates all site collections and their associated alerts, users, groups, and roles. It retrieves permissions for each user or group, expands nested memberships, and records detailed access relationships across SharePoint sites and subsites. The output is exported into a CSV file that provides a comprehensive overview of alerts and user access per site. The script supports authentication using interactive user login or app-based authentication and optionally includes OneDrive sites in the analysis.

.PARAMETER UseAppAuthentication
Specifies whether to authenticate to SharePoint Online using an app registration and certificate instead of interactive user authentication. Default is $false.

.PARAMETER IncludeOneDriveSites
Indicates whether OneDrive for Business sites should be included in the export. Default is $false.

.INPUTS
None. Configuration variables are sourced from the environment configuration script.

.OUTPUTS
CSV file containing alert information for each SharePoint Online site, saved to the configured data path ($AlyaData\sharepoint\AlertsPerSites.csv).

.EXAMPLE
PS> .\Export-AlertsFromAllSites.ps1 -UseAppAuthentication $true -IncludeOneDriveSites $true
...
Write-Host "SharePoint | Export-AlertsFromAllSites | O365" -ForegroundColor $CommandInfo
...
# Getting site alerts
Write-Host "Getting site alerts" -ForegroundColor $CommandInfo
$allSiteAlerts = New-Object System.Collections.ArrayList
foreach($site in $sitesToProcess)
{
    ...
    $siteAlerts = Get-PnPAlert -Connection $script:siteCon -AllUsers
    if (-Not $siteAlerts -or $siteAlerts.Count -eq 0)
    {
    ...
    $users = Get-PnPUser -Connection $script:siteCon

    Write-Host "  Getting list alerts"
    $lists = Get-PnPList -Connection $script:siteCon
    foreach($list in $lists)
    {
        Write-Host "    $($list.Title)"
        try {
            $alerts = Get-PnPAlert -Connection $script:siteCon -List $list.Title -AllUsers
        }
        catch {
            Write-Error $_.Exception -ErrorAction Continue
            continue
        ...
        {
            $alert = Get-PnPAlert -Connection $script:siteCon -List $list.Title -Title $alert.Title -AllUsers | Where-Object { $_.ID -eq $alert.ID }
            if ($allSiteAlerts.ID -notcontains $alert.ID)
            {
                $allSiteAlerts.Add([PSCustomObject]@{
                    SiteUrl = $siteUrl
                    ...
                    Filter = $alert.Filter
                    ListUrl = $alert.ListUrl ? $alert.ListUrl : $list.DefaultViewUrl
                    ListID = $alert.ListID ? $alert.ListID : $list.Id.ToString()
                    ItemID = $alert.ItemID
                    AlertFrequency = $alert.AlertFrequency
                    AlertTime = $alert.AlertTime
                    AlertTemplateName = $alert.AlertTemplateName.ToString()
                    AlwaysNotify = $alert.AlwaysNotify
                    UserLoginName = $alert.User.LoginName ? $alert.User.LoginName : $users | Where-Object { $_.Id -eq $alert.UserId } | Select-Object -ExpandProperty LoginName
                    UserId = $alert.UserId
                    Status = $alert.Status
                    ID = $alert.ID
                }) | Out-Null
    ...
    Write-Host "  Getting site alerts"
    foreach($alert in $siteAlerts)
    {
        $alert = Get-PnPAlert -Connection $script:siteCon -Title $alert.Title -AllUsers | Where-Object { $_.ID -eq $alert.ID }
        if ($allSiteAlerts.ID -notcontains $alert.ID)
        {
            $allSiteAlerts.Add([PSCustomObject]@{
                SiteUrl = $siteUrl
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAM2hp0eYsoYMoV
# Pcn67tWO/p/0/madrCXcV/AD+d5cgaCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE