...
<#
.SYNOPSIS
Installs or renews a SharePoint service application certificate from Azure Key Vault and imports it into the local machine certificate store.

.DESCRIPTION
The Install-ServiceApplicationCertificate.ps1 script automates the process of validating the existence of an Azure resource group and Key Vault, creating them if necessary, and managing a designated SharePoint RunAs certificate within the Key Vault. It ensures that the certificate exists, creates a self-signed one if missing, and securely exports and imports it into the local machine certificate store for SharePoint use. The script also sets appropriate access permissions in the Key Vault for the executing account.

.INPUTS
None. All required configuration values are imported from the Alya environment configuration file (01_ConfigureEnv.ps1).

.OUTPUTS
A certificate imported into the local machine personal certificate store.
A transcript log file written to the configured Alya logs directory.

.EXAMPLE
PS> .\Install-ServiceApplicationCertificate.ps1
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAiYoM1kdko8yWX
# zH9R0yu/L0GWugG2p+Rn5KOTOcP/PKCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEILzzNCMx
# UnpD34zKccULDDCc6vOIN5Bw1NQzrzOC41fMMA0GCSqGSIb3DQEBAQUABIICALRq
# sD3Ii/fpfUjRfTPFzstZTXWOrrySYWpQt6GrbiXLSVoL1oerjgUln5nl1YizicDG
# pwlWbaamdM4fJUVm/Ul+PIVeCubesZxP2qnAz1xMIsJIk4iWVySS4Th1nJSWUemU
# 5ms0OAVwl33FP0V0mZzKdUpsez4KMXNaPHwguQx44fJlmR0tX5cuxajrNDYt/1e1
# KYGBQ5LrRfFliwlaNOq0YMwXkwmydGy/AWQ6Wz5vqlRKGiwTpy5EDb9os6YNNgeE
# kwcYt6sLWcpV/irUQI37Vv/r9pQcucg+IvD4DV4J+Yxhq9+VqzKndQvWIsf0+Yf9
# wHYjGDejXC+zSrxgveCTfapnlyu2ROiUsKzBD5TK6EsvKxsi9NRGcruk5vuB8kmI
# sswqA9XTNcqRjXt5ZUDvuVhOQgG+06j5il2WmdEBmwTURH3YeFQNI1E5domK7gwV
# RpQfQauMLM5aN7fFzG4EBCO/2Rj3VlTMMz2sreJbtirKDq53N68STkMqsE9NXE/S
# GLCBjCMN1YJae9PSWaIIVVNY4XZQGdK/8W8HkU5r6w5A1wMXUf2c9eTD+lRS9/fW
# blc1xrRAT8zGfv7XjK1dNJ6okwbjRbylQtp8g+FGoCQwS2F/0EiAXtkbvv05ncbt
# rQU9WFnR0HzIQXgxG2wDIiuVRB7sSg/zWAxWZTtdoYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCD/NE91uy4o16SnAWaNeV7IenVtewMOUf/ZGM19e5A1FgIUAacm
# /JITnLwy/7R7iABtrgIMdHcYDzIwMjYwMjEyMjAzNDA1WjADAgEBoFikVjBUMQsw
...
# AQkEMSIEIBPNflF6G5ztBdNX9JxUmJNMzEK3hmSO0XpBi7EyFwZpMIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAk3DN
# WzZwQ3GpKhmvQmzzDlj/MOptTvEkKULfu6cLjP6nkG27/i6EJZHIsDDL8v5Bru8I
# TaNlR1hi2uIYIKqpsMffqJkn/xfDbolK31Oo8bJ+nApebm/T8SMg2NLLE+5H3kxw
# FohMPTzJy6/1UZSQQ2kVlzHcbN74+Q08/RUiWswW0EVe3B74bdQQbRR8RBsb4qTC
# fyR59pQZosIxHqM1NXpDAUCea0upDujPNCubnLuyrEVd8Fxv9vr2KVIyq8l8H5Y3
# tiI8jCwKdWV0A3afiLuqknWkTNY/lLftmuz4CEsv+YOWPeRTwNieYUhddbLtM1kP
# veeI7r1jy8gMa3jZSVRBKCbH0EI+ykFXBm0BTE8OIY2nGecabSR79hAt0cfTtz7V
# COdTxtCLznH4SoVzznmRJ/UTGgpcqG9hoYg1tR1i+w0oLGSs8Vw2VUIcCDIOiQWf
# 3d9KtzFTHVnWnBarJCLyNSck8EOPiII0T35aABIevWrbtBhrRwrQiqpIiM53
