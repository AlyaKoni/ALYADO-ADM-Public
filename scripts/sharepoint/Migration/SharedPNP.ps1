...
$AlyaSharePointAdminUrl = "https://customer-admin.sharepoint.com"
$AlyaSharePointUrl = "https://customer.sharepoint.com"
$AlyaSharePointOnPremUrl = "https://webapp.customer.root"
$AlyaSharePointOnPremMySiteUrl = "https://my.customer.root"
$AlyaPnPAppId = "11111111-2222-3333-4444-55555555555"
$AlyaTenantName = "customer.onmicrosoft.com"
...
function LoginTo-PnP-OnPrem(
    [string] [Parameter(Mandatory = $true)] $Url
)
{
    Write-Host "Login to SharePointPnPPowerShellOnPrem '$($Url)'" -ForegroundColor Cyan
	...
	if ($null -eq $Global:AlyaSharePointOnPremCredentials)
	{
		try {
			$AlyaConnection = Connect-PnPOnline -Url $Url -ReturnConnection -TransformationOnPrem -CurrentCredential -ErrorAction Stop
			...
			Write-Warning $_.Exception.Message
			Write-Warning "Login with current credential was not working."
		}
	    $Global:AlyaSharePointOnPremCredentials = Get-Credential -Message "Please provide SharePoint On-Premises admin credentials"
	}

	try {
		$AlyaConnection = Connect-PnPOnline -Url $Url -ReturnConnection -TransformationOnPrem -Credentials $Global:AlyaSharePointOnPremCredentials -ErrorAction Stop
        [object[]]$Global:AlyaPnpConnections += $AlyaConnection
		return $AlyaConnection
	}
	catch {
		Write-Warning $_.Exception
		Write-Warning $_.Exception.Message
		Write-Warning "Login with provided credentials was not working."
	}
...
{
    Write-Host "Login to SharePointPnPPowerShellOnline '$($Url)'" -ForegroundColor Cyan
    ...
    if ($null -eq $AlyaConnection)
    {
        if ($ClientId)
        {
            ...
            {
                try {
                    $AlyaConnection = Connect-PnPOnline -Tenant $AlyaTenantName -Url $Url -TenantAdminUrl $TenantAdminUrl -ReturnConnection -ClientId $ClientId -CertificatePath $CertFile -CertificatePassword $CertPassword -ValidateConnection
                }
                catch {
                    $AlyaConnection = Connect-PnPOnline -Tenant $AlyaTenantName -Url $Url -TenantAdminUrl $TenantAdminUrl -ReturnConnection -ClientId $ClientId -CertificatePath $CertFile -CertificatePassword $CertPassword
                }
            }
            else
            {
...
$mod = Get-Module -Name "PnP.PowerShell" -ListAvailable
if (-Not $mod)
{
	Write-Warning "Module PnP.PowerShell not found. Installing in scope CurrentUser"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAlTZ/tXQ+JH4Ll
# QUPylR4bZzwfUz7Te0OyjfDlRAZwm6CCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE