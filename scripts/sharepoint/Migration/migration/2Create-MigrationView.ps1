...
Start-Transcript -Path "$root\logs\Create-MigrationView-$($TimeString).log" | Out-Null
...
foreach ($webAppKey in $expSites.Keys) {
    try {
        ...
        $webApp = Get-SPWebApplication -Identity $webAppKey 
        if (-not $webApp) {
        ...
        foreach ($siteKey in $expSites."$($webAppKey)".Sites.Keys) {
            try {
                ...
                $site = Get-SPSite -Identity $siteKey
                if (-not $site) {
                    ...
                    try {
                        ...
                        foreach ($listKey in @("Documents")) {
                            try {
                                ...
                                $toMigField = $fields.GetEnumerator() | Where-Object { $_.Value.InternalName -eq "MigAttrName1" -or $_.Value.InternalName -eq "MigAttrName2" }
                                $migView = $list.Views["MigrationView"]
                                if (-Not $migView) {
                                    Write-Host "Creating MigrationView in list $($listKey) in Web $($webKey) in Site Collection $($siteKey) in Web Application $($webAppKey)."
                                    $viewFields = $list.Views["All Documents"].ViewFields.ToStringCollection()
                                    $strQuery = "<Where><Eq><FieldRef Name='$($toMigField.Value.InternalName)'/><Value Type='Boolean'>1</Value></Eq></Where>"
                                    $result = $list.Views.Add("MigrationView", $viewFields, $strQuery, 100, $true, $false , "HTML", $false)
                                }
                                else {
                                    Write-Host "MigrationView already exists in list $($listKey) in Web $($webKey) in Site Collection $($siteKey) in Web Application $($webAppKey)."
                                }
                            }
                            catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDgFdY2OIQp/S/x
# KSiodJ6b8Z/d64NtEaX93FdQ5N09M6CCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIDQ0BGYP
# kM6umYwMwqqRd+csBO7eBX153mvMEFXWeYETMA0GCSqGSIb3DQEBAQUABIICAEWe
# tw83MenAZn6c6APoor6UjOpWv4TWaXCRek6gZgIVsrZz1LX1wjyPkamyx9nUgQK2
# MA6ZXsp4Gz/ttkcOlhQkiq2eZsLpf97wBwiy8eF65YMv/1+HMv37f1uvFrjSGWIY
# dlrq9J7jshd6DcWNiy+HJDPqgeQeloms/vH2MFAsILcSyPMfn1Fm/KYLvXO8yDk5
# pKUfxtXrSmhISGe1iVousQYdRIH+DYnjMH6pinOTe1ESw6195NlDwRMNsICZDpME
# IhAox8uOIQYRTSBOrMcTpq5ObxMOycL+Bzco+/vWyAMCxcEnaIUyO2lyxRXyKlER
# qe/cKGD/NW+OYTNWTLAjEwSglcH8S1GXn/E40kvHRz6xmyBhaQRmN+oOkBIwDjlA
# HKCDFFknRpcBBzltciTtrnkBVd7zQrOOBpPHc4hDm1MnUKo6Z5HG+Z6w4hAFiVIB
# EgRwhGgpMAmxkdbrYQTrhPeXVVJU1EnacovZ8RkwrYODQq8V42je6ryOfiIEB9QS
# 8kqUAyL9xHQmvfqfQ47PtJSX0fFWF9jsTT3JXc7zScXppNCEfq3SCkKj7FrOqOZ6
# wRBuljIvuibxRTgVoH6AB0tTrOcfhsjD4KQD7o47c7qv/5CxVdeddf38/MyMtc9u
# fqbbkXcGTWcLUQ1pCcwVtZejlW0oYg/T1e3+IPzcoYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCAfOIpHQ4+Got92jbGimEMx81jOc59zKH3/KrwGbybg0gIUfnyx
# ekCyE5zLcaksS/0D241iwzoYDzIwMjYwMjI2MTU0MzE2WjADAgEBoFikVjBUMQsw
...
# AQkEMSIEIDyw/zu6CGK0T5SyZRmJm/I2U8pdbW/NH4xvil+pVN4CMIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGANQ9/
# qmzyD2aoDazneYDRrW2Xe1CVWWdPJ+/eCV5OEGf5pFJYAICJN2R03ETMVyXSM5tE
# FQDYxjg3LnhPw6s2rm2N7Wy7C+dOBCO9qzyq8hIsfWFvahTtDbYq6eoDvl///vv9
# CCt4h3OhOv15tCIVvDs1i9O0V2mVdQ8FRtRLPGwdsl2l4XCN8BhmERfRnmOaPIrf
# nzZAVl2O7Q6e1VapDfkbwZw/jkogN0/+5LllSNCEV2cUuz9UWzM9HJHM5NyT2l0o
# 1j8s34LHdP/lQ8wNVuTfx4zaKDuT/QmfNDoqxdQi7mhkSmew+uuDmroq90n6dbGs
# iuABrNvSyzmrK6E7O8dZMsqoDoYbUw6RaxK+pm7Rw4UW25NDjl22FKt6f7cEsemK
# kNLHbi1DFC4mbdGvWCGCdW8v8yyGbVFn5jspHp4hK7eaVvnrQqAEH3BvSMtLAhxi
# hfzWeVZn52AlTy8GUfrjRhWBSLagKwyWIeorVo2pYUbUX4cXCGdapfB4FhEJ
