...
# Starting PnP Server
#Write-Host "Starting PnP Server"
#$proc = Start-Process pwsh.exe -PassThru -Arg "-nologo -noprofile -file `"$root\PnP-Server.ps1`""
...
if (-Not $admincon)
{
    $admincon = Connect-Site -Url "https://customer.sharepoint.com" -Browser -DisableSSO -ProxySettings $proxysettings #TODO test with $migUser
}
$allData = @()
...
foreach($listName in $listNames) {
    foreach ($SiteMapping in $SiteMappings) {
        ...
        try {
            ...
            #Write-Host "  Special sharegate bug fix"
            #& "$root\PnP-Client.ps1" -message $dstSiteUrl
            ...
            while ($retries -le 5 -and $result.Errors -gt 0) {
                ...
                if ($DryRun) {
                    ...
                    if ([string]::IsNullOrEmpty($dstListFolder)) {
                        ...
                        {
                            $result = Copy-Content -SourceList $srcList -DestinationList $dstList -CopySettings $migCopySettings -MappingSettings $migMappingSettings -Template $migTemplate -NormalMode:$false -WaitForImportCompletion -Verbose -Debug
                        }
                    } else {
                    	if ($listName -ne "Images")
                        {
                        ...
                        {
                            $result = Copy-Content -SourceList $srcList -DestinationList $dstList -DestinationFolder $dstListFolder -CopySettings $migCopySettings -MappingSettings $migMappingSettings -Template $migTemplate -NormalMode:$false -WaitForImportCompletion -Verbose -Debug
                        }
                    }
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAlFmmDsR7tEtIz
# d4lg8a9jxAaef+5FLWmQp5nQ0bVFrKCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIE7F5Mzh
# QJ980+bzGkkg/g0OALaNFQCHT/J3L3WDN0O3MA0GCSqGSIb3DQEBAQUABIICAL43
# cPM6g//C5A/K7jmMF3oj3pcI5imAR2diQE0mGJHM0u5KABiSzRMo+3myFzIyiN5C
# aCnercbpY2rRgvv5rhXwClJUj4Ws51aNrB6EjhZHzWXJ1yBulD26QKeAuRoYtCSF
# E0x5VBN0+GbGUM9z0K8XfWT7GO4pQ4+yJ7wZiHoh6oaL5BZbb7oH7TXwP/Mp5Gaq
# vpCk35Is3pdHdHQug8q8Bba2uCYSojWN+Bb7ATAOqj7wk2xDRX5bmCo9PPEQJxa5
# kkgz0jVC8nW1TJC2M4+6EnoSyUqexmwETFNOR1yNG/hSEEotfq+Hi2s9pto+uUgs
# 8ulHqfoFekw4UrQEtWy7kHaUzztVWnC+l+e7TRSUf4jj02M/OZ1ukuQVB5KEe/M5
# 4QfaSE+dvmfXM/gSZOY2/V4+cJ/ITL33dUUI0l7IiuBpiM5N1aSpq33fwhPOlAo4
# ++K58r9QXxKSTDu9XNy6S80P0KZMfBDJKMtNyGl+KGzYwlvfYnOC1bd18NaE6D01
# VvDRuSCHC0c2b7nWNwdtCBv+c54Cy/rulNojbya3BBttcokRQDhnIsbVu9VCJ1Gy
# rSgqAYtcP3lING0REo9wc+UpqjdT8q5OUr9ZLD0S9yB9asNIU5QCg9uNTJud/cpS
# qaL47L/ExVw2L864RJWJC+PoI3YdMwSXGaGnpZBMoYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCAGq1NdbwoV6zG87uDBqUFmbHkKw38mVHOaemE2qPuLHgIUaElX
# e5M4QkbDsvHFlwOlBlbk+88YDzIwMjYwMjI2MTk0MzI1WjADAgEBoFikVjBUMQsw
...
# AQkEMSIEIFIm1fLEYLKyKzdYnEO6uCeMNHaqgUOsZ8NNc6mAJIAdMIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAngZU
# LZ7hsjAmBIacu62awB1KaORAIeQHAScEoKWUxIu7r8HXakJWTITR0X6nuCs9Nu+u
# Ru/QVpyxywPQCc8HlFYpOWZsCGsadqOy4cQTusq7JS5YCGG9HyOqOzkWq4rPFcCT
# Mx58YC4csrkIL9s1tqV00bbzROA7jPLHoNox0rvRxNAIcfbnuHVKG7DrsETTFJYE
# lGX2rQ6OVtMKTJseML+5+akxuRdDws74o12kzlXGJflD9K2Klfy/qeg/1StPltuv
# Cie9gMentE/58zAypjeqeAgRTmHIAB8/4N6pNaBHIfaptRwflcYDpBt0kLFem2kp
# CJZ762g+B17lB0bifM1cW53JdJQr9ub5hobsAKaMT+L+gMYjLAugBvSt8/uEKuNI
# MLPtKEx/bX/s1dkKTv2bXUJqw56g2co6pqjXgp26/ppgNF0ROX+BWJbA5utNqn33
# F5mV4ttZID6gBWEGYknIPpeEugIqws56S6sOhk8ddH7qur6B/XvYG0mQXV+L
