...
# Starting Get-Taxonomy
Write-Host "Starting Get-Taxonomy"
$root = $PSScriptRoot
...
Start-Transcript -Path "$root\logs\Get-Taxonomy-$($TimeString).log" | Out-Null
...
function Export-SPTermSet
{
    ...
    if ($terms.Count -gt 0)
    {
	    $TermsElem = $Global:resultInXml.CreateElement("Terms");
	    $TermsElem.SetAttribute("Level", $level)
	    $null = $setElement.AppendChild($TermsElem);
    }
    ...
    if ($level -ge 1 -or $level -le 15)
    {
        if ($terms.Count -gt 0 )
        {
            ...
            foreach($term in $terms)
            {
			    ...
			    $TermElem = $Global:resultInXml.CreateElement("Term")
			    ...
			    $TermLabelsElem = $Global:resultInXml.CreateElement("Labels")
			    ...
			    foreach ($TermLabel in $TermLabels)
                {
				    ...
				    $TermLabelElem = $Global:resultInXml.CreateElement("Label")
			    ...
			    $TermCustomPropertiesElem = $Global:resultInXml.CreateElement("CustomProperties")
                ...
                {
				    $TermCustomPropertyElem = $Global:resultInXml.CreateElement("CustomProperty")
			    ...
			    $TermLocalCustomPropertiesElem = $Global:resultInXml.CreateElement("LocalCustomProperties")
                ...
                {
				    $TermLocalCustomPropertyElem = $Global:resultInXml.CreateElement("LocalCustomProperty")
                ...
                #if ($level -lt 15)
                #{
                    Export-SPTermSet -setElement $setElement -terms $term.Terms -level ($level + 1) -previousTerms ($previousTerms + $currentTerms)
...
# Create xml
$Global:resultInXml = New-Object xml
$decl = $Global:resultInXml.CreateXmlDeclaration("1.0", "UTF-8", $null)
$Global:rootNode = $Global:resultInXml.CreateElement("ManagedMetadataExport")
$Global:rootNode.SetAttribute("ExportType", "Global")
$Global:rootNode.SetAttribute("Created", ((Get-Date).ToString("o")))
$null = $Global:resultInXml.InsertBefore($decl, $Global:resultInXml.DocumentElement)
$null = $Global:resultInXml.AppendChild($Global:rootNode);

# Processing
Write-Host "Processing"

$taxSite = Get-SPSite (Get-SPWebApplication -IncludeCentralAdministration | Where-Object { $_.IsAdministrationWebApplication -eq $True }).Url
...
foreach ($termStore in $termStores)
{
    ...
    $TermStoreElem = $Global:resultInXml.CreateElement("TermStore")
    ...
    $null = $Global:rootNode.AppendChild($TermStoreElem)

    #Write out TermStoreAdministrators as subcategory of TermStore
    $TermStoreAdministratorsElem = $Global:resultInXml.CreateElement("TermStoreAdministrators")
    $null = $TermStoreElem.AppendChild($TermStoreAdministratorsElem)	 
    foreach ($Administrator in $termStore.TermStoreAdministrators)
    {
        $TermStoreAdministratorElem = $Global:resultInXml.CreateElement("TermStoreAdministrator")
    ...
    #Write out TermGroups
    $TermGroupsElem = $Global:resultInXml.CreateElement("TermGroups")
    $null = $TermStoreElem.AppendChild($TermGroupsElem)
    $termGroups = $termStore.Groups
    foreach ($termGroup in $termGroups)
    {
        Write-Host "Exporting termgroup $($termGroup.Name)"

        # Write out the TermGroup properties
        $TermGroupElem = $Global:resultInXml.CreateElement("TermGroup")
        ...
        $TermGroupElem.SetAttribute("Name", $termGroup.Name)
        $null = $TermGroupsElem.AppendChild($TermGroupElem)

        # Write out all managers of TermGroup as subcategory of TermGroup
        $TermGroupManagers = $termGroup.GroupManagerPrincipalNames
        $TermGroupManagersElem = $Global:resultInXml.CreateElement("GroupManagers")
        $null = $TermGroupElem.AppendChild($TermGroupManagersElem)
        foreach ($TermGroupManager in $TermGroupManagers)
        {
            $TermGroupManagerElem = $Global:resultInXml.CreateElement("GroupManager")
        ...
        $TermGroupContributorsElem = $Global:resultInXml.CreateElement("Contributors")
        $null = $TermGroupElem.AppendChild($TermGroupContributorsElem)
        foreach ($Contributor in $termGroup.Contributors)
        {
            $TermGroupContributorElem = $Global:resultInXml.CreateElement("Contributor")
        ...
        $TermSetsElem = $Global:resultInXml.CreateElement("TermSets")
        ...
        {        
            ...
            $TermSetElem = $Global:resultInXml.CreateElement("TermSet")
            ...
            $TermSetCustomPropertiesElem = $Global:resultInXml.CreateElement("CustomProperties")
            ...
            foreach ($Key in $TermSetCustomPropertyKeys)
            {
                ...
                $TermSetCustomPropertyElem = $Global:resultInXml.CreateElement("CustomProperty")
            ...
            Export-SPTermSet -setElement $TermSetElem -terms $termSet.Terms

        }
    }
}

$Global:resultInXml.Save("$root\data\Taxonomy.xml")
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBb7cvuVw/RYnAt
# 6128K3jfkkrkhZ6DLdWlLHznRGh7T6CCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEICI3YKXO
# 602jT9LhzkdAUx1uOKIuIa4p1a5yeY0d1Av/MA0GCSqGSIb3DQEBAQUABIICADyj
# BzNzvJEiiBMISm1bkPag/BJ+W7PEIhETCy1uBJLTQCnxtLul9m2Jcx1udWzvpevJ
# ytrVmqPzvWxtTBjrhJuQzWAM+u+iY0vUg73LFTLw3a02IB0vfQlxu3PgwgJbqd6j
# DTPNpn2ifkhWTNSxLo5NqQVfVXcTWn7tW0KHRD0nm5GvXMKkwiMPN9Cdcjt2F48+
# ZS6f25pWotqFg/0gUcbfrm3nF4BtgJqyTl64YuVmh+KQmJKdLX1T5jWY5YWGEXWg
# qh9AVbbeWXeyXNLBhQXDypRUGwLP3OQydGc+ZMrq16Ws4axUvZzNqVBZm7vfC0db
# oQp9HVH2BjbvHuo54hjzT734a8IqDmDX26GcnAnG0ogwu3wG5bIZY4Xo5qKjMIUM
# 396GCanDlvfalFdSzkUne7ydx7Mpkcx1qO5uP808BoDwB8JYJz5ZQzVbtflUKvEk
# h6KTC5hb2jKcwJBLQlXv8ovhfJnLML7o/kpAThgOng7v7Dxs8V2b4rzCRVj//4bU
# zlbXGcKkRNVCQbJ5kAumtSRChiFzAV7OVt1Lw5SnwqOnZk6bEw2WqnPb5ibLYp34
# ryCPSAZsRTsv3orPthQ0bzCK5V1I+EtzR9do/rYcPxGW+AaEOvq9asBz24HMTduD
# yGJBRjcZs6JJ6HcW/C4BV0qLPbJkaS4x0TloRbN/oYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCBqOgFUcMYpEgsXCUilsS18Ft5H4TsLldQhgC8VYmjJJwIUTYU3
# ZLigIcbt+JIbTBSJiH6EC4AYDzIwMjYwMjA0MTQzNzMyWjADAgEBoFikVjBUMQsw
...
# AQkEMSIEIAxIAS3pS6hfzQjlefEPvD7IeCHrV4ErJo9Ynf+nueF1MIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAh6Md
# 2jtyy/rcAs2jHgm/2CxfiaONE0Lu6bvcHtyBIP8ptBP+v2dQSwQCM1Wp+4F/LEFk
# T/nq/EwUJo4KcBDUoiHWmS+q/p1In+tG6+52gHUxHHaDnMvyPDq5k4ZgpAhiIUlB
# ctawx7goAXSRm1EYQa8pCfZzPx6AUq1xZUx6+ObK4aDCCYc8P9ETdoA6qwSK5TgC
# vmsGmjQ3qv0KcMtgbEWeVTTbkmShyPMwTbWgi0ADom6Yl1oHpRTR4MvlIKzPxYbA
# h1iXn5H8UQuNuxJdd+SfOq1QIe3uaWEHd1ldw3uf5LUZGTzirw19PjKEGIIO23VB
# ruvFR5HXpL3Co7z7SePk1aae2bK6gtj14bZcOe4hiKJtQakrEEsXYCUhqGFa+NJN
# MhmuRYEbXvG1tmiZ0j1Tcv6R+Sa3mdMSPadivx3yC9iZeq69WQvJVDzwItZjuiOI
# f4q17IVKzr2kd64+kspSWk5KCoJXZ9/RYFZTK6+OlPgbNZ752p+y5u5gjMVC
