...
Start-Transcript -Path "$root\logs\Get-Sites-$($TimeString).log" | Out-Null
...
Get-SPWebApplication | ForEach-Object { 
    ...
    try
    {
        Write-Host "WebApplication: $($webApp.Name)"
        $allData."$($webApp.Name)" = @{
            Id              = $webApp.Id
            Name            = $webApp.Name
            DisplayName     = $webApp.DisplayName
            DefaultTimeZone = $webApp.DefaultTimeZone
            Status          = $webApp.Status
            TypeName        = $webApp.TypeName
        ...
        foreach ($siteCol in $sites)
        {
            try
            {
                Write-Host "  SiteCollection: $($siteCol.Url)"
                $allData."$($webApp.Name)".Sites."$($siteCol.Url)" = @{
                    ID                 = $siteCol.ID
                    CompatibilityLevel = $siteCol.CompatibilityLevel
                    HostName           = $siteCol.HostName
                    Port               = $siteCol.Port
                    PrimaryUri         = $siteCol.PrimaryUri
                    Protocol           = $siteCol.Protocol
                    ServerRelativeUrl  = $siteCol.ServerRelativeUrl
                    SchemaVersion      = $siteCol.SchemaVersion
                    Webs               = @{}
                }
                $webs = Get-SPWeb -Site $siteCol -Limit all
                foreach ($web in $webs)
                {
                    try
                    {
                        Write-Host "    Web: $($web.Url)"
                        $allData."$($webApp.Name)".Sites."$($siteCol.Url)".Webs."$($web.Url)" = @{
                            ID                = $web.ID
                            Name              = $web.Name
                            Title             = $web.Title
                        ...
                        foreach ($list in $web.Lists)
                        {
                            try
                            {
                                ...
                                $allData."$($webApp.Name)".Sites."$($siteCol.Url)".Webs."$($web.Url)".Lists."$($list.Title)" = @{
                                    ID                     = $list.ID
                                    Title                  = $list.Title
                                ...
                                foreach ($field in $list.Fields)
                                {
                                    try
                                    {
                                        $allData."$($webApp.Name)".Sites."$($siteCol.Url)".Webs."$($web.Url)".Lists."$($list.Title)".Fields."$($field.InternalName)" = @{
                                            Id                  = $field.Id
                                            Title               = $field.Title
                                            ...
                                            FillInChoices       = $field.FillInChoices
                                    ...
                                    {
                                        Write-Warning "Error processing field $($field.InternalName) in list $($list.Title) in web $($web.Url) in site collection $($siteCol.Url) in web application $($webApp.Name): $_"
                                    }                
                                }
                            ...
                            {
                                Write-Warning "Error processing list $($list.Title) in web $($web.Url) in site collection $($siteCol.Url) in web application $($webApp.Name): $_"
                            }
                    ...
                    {
                        Write-Warning "Error processing web $($web.Url) in site collection $($siteCol.Url) in web application $($webApp.Name): $_"
                    }
            ...
            {
                Write-Warning "Error processing site collection $($siteCol.Url) in web application $($webApp.Name): $_"
            }
            try { $siteCol.Dispose() } catch { }
        }
    
    }
    catch
    {
        Write-Warning "Error processing web application $($webApp.Name): $_"
    }
    try { $webApp.Dispose() } catch { }
}
$allData | Export-Clixml -Path "$root\data\Sites.xml" -Force
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCkCI70LUUDVjiQ
# 14V7aJqLIQ7nkp8sC2wJEOxa2+v+caCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE