...
# Starting Get-Peoples
Write-Host "Starting Get-Peoples"
$root = $PSScriptRoot
...
Start-Transcript -Path "$root\logs\Get-Peoples-$($TimeString).log" | Out-Null
...
try
{
    ...
    foreach ($expProfile in $expProfiles)
    {
        try
        {
            Write-Host "$($expProfile.AccountName) $($expProfile.UserName) $($expProfile.WorkEmail)"
            ...
            try
            {
                $profile = Get-PnPUserProfileProperty -Connection $adminCon -Account $accountName
            }
            catch
            {
                try
                {
                    $profile = Get-PnPUserProfileProperty -Connection $adminCon -Account $expProfile.AccountName
                }
                catch
                {
                    try
                    {
                        $profile = Get-PnPUserProfileProperty -Connection $adminCon -Account $expProfile.WorkEmail
                    }
                    catch
                    {
                        try
                        {
                            $profile = Get-PnPUserProfileProperty -Connection $adminCon -Account $expProfile.UserName
                        }
                        catch
                        {
                        }
            ...
            if (-not $profile)
            {
                ...
                $userDataMissing += $expProfile
                continue
            }
            $userData += @{
                ID          = $profile.UserProfile_GUID
                UserID      = $profile."msOnline-ObjectId"
...
$userData | Export-Clixml -Path "$root\data\Peoples.xml" -Force
$userDataMissing | Export-Clixml -Path "$root\data\PeoplesMissing.xml" -Force
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBN25K5+xRZn6bi
# /OhegjYtgSKyGjY9u5DIxr8Us+mw4aCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIHPyqRyJ
# Zt6panWD11LGPx2JujjNEt2SxU/L1amM3zlmMA0GCSqGSIb3DQEBAQUABIICAHaC
# 1YCyqo3o4lDbwjKg7BDC4mgy1+YVo5UkhomgZI0FYgTBQ95BYGm+PsRjqEXINIH2
# Ps2d6F0UqwtclXJaS917eTmEr9mPLSiVTuv7sSnNuc5xbaczPm/HTo3pgEA6SZO6
# 5Nvh938TX9F+kTRN0XutyFq1WVGV/3VYx5zbapmHpRpnFsUW9YMwGdfippje8kb4
# pVP3F1mGwIJgwnOGEZK0HR57JY73fQFRGxh3aDsWg9wLqr0IE+e9eimOMZD1Macz
# YWtNHFGTDTPLR7EsHdfKuh5zGu1oM2OtOwCR5FtPTM/tIJBa8Oiwp6l+guWt0yhE
# 4QthL3Z8uF+qB/gF+HVCvkfhIJXYlJ2FhkaNsyIuYJq7BXXgVq1q7n1JbAVLNTU8
# ugGl9gqVVxvXa8b2JpwxCNL90GVTZXEuOqv0csf/B8O7BjLayUXnpJa7yJfZvHvA
# U/gSoY0/pgTkjh33e7m2/P/ySYpWgAPfpX8fpnRifqfA+prEFWr+tI8aeXL1xfvS
# wtBGzCkm5PWoBGORrKhavJDs/82QP6q1Pyw30dyU4LUgwW+Qx4vwc3b+SYDcry1w
# /r4rn3Z67C6CNW8dFV+mL4QKZ5BCR8YQvRuEwsSTHQ4L0zS9rxFA8oTKuiyK9GiE
# CWkcoShuh2TS8FJsecHCGYHX4RfgPOXN59KZ630XoYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCCT1p63AqEXDfClqk/1F1foyuEGDaDnzTwI0s9rrZLAPwIUUdUA
# HJfgNRPAqj3pg2Goxcy643AYDzIwMjYwMjA2MTM0NTAwWjADAgEBoFikVjBUMQsw
...
# AQkEMSIEINmg0QJ2+3o1zMJSjfjIH5SkZxitWgO9o8sKUKegVGu3MIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAPerB
# KlUKEdYDwz8pcrhtBl8383P0Y7C8ODqTG84sSYjXnuzTXOl/VD0G39rNNlgyDZI3
# PiTjOefRxJN4t5YWvE1rcxMlbwTvJtzObvryB5DOj4vt5S5XIIKm3A2v0ZHHV0/l
# kt3HFUf62CVqKvaWmQXbjQu2hybcRUSjYn4Tcdukla/30Hc42ct2hg2QvZ0canlm
# A0wOD5BwP9GLMs1oBY2xGRCjsh74TwEuwgDNwCdoI++AIacWYwOb93rI9yCvPvrz
# tCsvRpLC3vlE+vm8jDrhlFkYtDd92WkZg/ffA7NJ5VluHUEaLdOiB87gHdcKaYlY
# oVdjkMzCSDGelESc5zo5nKj+NETYrKV9n+3b4o9Lqdaqg93hXCisOcZnCOwsMFrW
# iTtUU0sQVRJ0Y/Yj2oC6iJdNBz2LZvneN3UeBYiBB1JOn5od2wq35ua7581CIZjg
# T3Umpd/obAg075c9rKq3I7G9ePwEv0b/VCOMvvmr8pIn+HnAj+YRGwb3SgFc
