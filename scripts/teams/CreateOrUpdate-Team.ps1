...
<#
    ...
    09.09.2024 Konrad Brunner       Added MembersDynamicRule parameter
...
<#
.SYNOPSIS
Creates or updates a Microsoft Teams team with specified configuration including members, owners, guests, and channels.

.DESCRIPTION
The CreateOrUpdate-Team.ps1 script allows the creation of a new Microsoft Teams team or the update of an existing one. It verifies module installations, connects to Microsoft Graph and Teams, and ensures that the team’s configuration aligns with the provided parameters. The script supports adding owners, members, guests, dynamic membership rules, and additional channels. It can also update team descriptions, visibility, logos, and guest access settings.

.PARAMETER TitleAndGroupName
Specifies the display name of the team and group. This parameter is mandatory.

.PARAMETER Description
Provides a description for the team. Defaults to an empty string.

.PARAMETER Visibility
Specifies the visibility of the team. Valid values are 'Private', 'Public', or 'HiddenMembership'. Defaults to 'Private'.

.PARAMETER Owners
Specifies an array of users or groups to assign as team owners.

.PARAMETER Members
Specifies an array of users or groups to add as team members.

.PARAMETER MembersDynamicRule
Specifies the dynamic membership rule for the team group. If provided, dynamic membership is applied.

.PARAMETER Guests
Specifies an array of guest user email addresses to add to the team.

.PARAMETER AddChannels
Specifies an array of channels to create or update within the team. Each channel can include properties such as DisplayName and Description.

.PARAMETER OwerwriteMembersOwnersGuests
When set, existing team owners, members, and guests are overwritten with the specified ones.

.PARAMETER TeamPicturePath
Specifies the file path to the image used as the team’s logo.

.PARAMETER AllowToAddGuests
Specifies whether guest users can be added to the team. Defaults to $true.

.PARAMETER AutoInviteGuests
If enabled, automatically invites users not found in the directory as guests. Defaults to $true.

.PARAMETER InviteGuestsSendInvitation
Specifies whether to send an invitation email when inviting guests. Defaults to $true.

.INPUTS
None. The script accepts parameter inputs directly.

.OUTPUTS
None. The script performs operations and logs progress to a transcript file.

.EXAMPLE
PS> .\CreateOrUpdate-Team.ps1 -TitleAndGroupName "Marketing Team" -Description "Internal team for marketing projects" -Visibility Public -Owners "owner@contoso.com" -Members "member1@contoso.com","member2@contoso.com" -AddChannels @(@{DisplayName="General"};@{DisplayName="Campaigns";Description="Marketing campaign discussions"}) -AllowToAddGuests $true
...
Param(
    ...
    [string[]]$Members = @(),
    [Parameter(Mandatory=$false)]
    [string]$MembersDynamicRule = $null,
    [Parameter(Mandatory=$false)]
    [string[]]$Guests = @(),
    [Parameter(Mandatory=$false)]
    [object[]]$AddChannels = @(),
...
if ([string]::IsNullOrEmpty($MembersDynamicRule))
{   
...
{
    $grp = Get-MgBetaGroup -GroupId $Team.GroupId
    $GroupTypes = $grp.GroupTypes
    if ("DynamicMembership" -notin $GroupTypes)
    {
        $GroupTypes += "DynamicMembership"
    }
    Update-MgBetaGroup -GroupId $Team.GroupId -GroupTypes $GroupTypes -MembershipRule $MembersDynamicRule -MembershipRuleProcessingState "On"
}
