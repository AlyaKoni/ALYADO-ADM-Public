...
<#
.SYNOPSIS
Creates or updates Windows Virtual Desktop (WVD) application groups and associated remote apps based on predefined configurations.

.DESCRIPTION
This script automates the management of WVD application groups within an Azure environment. It checks necessary Azure modules, authenticates with Azure and WVD using service principal credentials stored in Key Vault, deletes an existing default desktop group, and creates or updates custom application groups and their associated remote apps as defined in input arrays. It also manages access by assigning or removing users and groups to the appropriate application groups. Logging is performed to capture execution details for auditing or troubleshooting.

.PARAMETER HostPoolName
Specifies the name of the WVD host pool to which the application groups and apps belong.

.PARAMETER appDefs
An array of application definitions, where each item includes app name, file path, command line settings, and icon details used to create remote apps.

.PARAMETER appsToGroup
An array defining which applications should belong to which application groups, including user access information for each group.

.PARAMETER availableIcons
A collection of available icon names that can be applied to remote apps during creation.

.INPUTS
None. The script accepts parameters through the Param() block.

.OUTPUTS
None. The script writes progress and status information to the console and a log file.

.EXAMPLE
PS> .\15_createOrUpdateAppGroupsApp.ps1 -HostPoolName "ProdHostPool01" -appDefs $appDefs -appsToGroup $appsToGroup -availableIcons $availableIcons
...
$AzAdApplication = Get-AzADApplication -DisplayName $AlyaWvdServicePrincipalNameProd -ErrorAction SilentlyContinue
...
$AzAdServicePrincipal = Get-AzADServicePrincipal -DisplayName $AlyaWvdServicePrincipalNameProd | Where-Object { $_.AppId -eq $AzAdApplication.AppId }
...
if (-Not $Global:RdsContext)
{
	...
	$rdsCreds = New-Object System.Management.Automation.PSCredential($AzAdServicePrincipal.AppId, $AlyaWvdServicePrincipalPasswordSave)
	...
	#LoginTo-Wvd -AppId $AzAdServicePrincipal.AppId -SecPwd $AlyaWvdServicePrincipalPasswordSave
}

# Deleting app group
Write-Host "Deleting app group $AppGroupNameToDelete" -ForegroundColor $CommandInfo
$appToDelete = Get-RdsAppGroup -TenantName $AlyaWvdTenantNameProd -HostPoolName $HostPoolName | Where-Object { $_.AppGroupName -eq $AppGroupNameToDelete }
if ($appToDelete)
{
...
#Get-RdsStartMenuApp -TenantName $AlyaWvdTenantNameProd -HostPoolName $HostPoolName -AppGroupName "Desktop Application Group" | Select-Object AppAlias
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCKAyOwwo8AcoNy
# 62E/eIZATgqqFROOlw/PDiTiQgab/aCCDuUwggboMIIE0KADAgECAhB3vQ4Ft1kL
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIAzTZLd44Z2u6+mX
# x7+bT0RdR+bzz5dYuWh13oN952OFMA0GCSqGSIb3DQEBAQUABIICAKCucwuXYCTi
# HI6gyDdlSr0356O1DNG9tCMfaQ6+rkVM9Tih7lfBwjCEIc1T6DHhBpZIYdS1OXyt
# 2ugOHJCze8bdhAZ7TvuqOh813Rx36S/N611XwrbT6GKapYHrgaiyzkwBWqJGUyTq
# cmtiEeZwTCdLG1bmfAWT4+eQzMjk+89Mtw9LUF+/Qjupml+kbdhnNX6Ihj9HuGiv
