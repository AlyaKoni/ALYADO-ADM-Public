...
Start-Transcript -Path "$($AlyaLogs)\scripts\wvd\admin\prod\02_createServicePrincipal-$($AlyaTimeString).log" | Out-Null
...
{
    Write-Warning -Message "Azure AD Application not found. Creating the Azure AD Application $AlyaWvdServicePrincipalNameProd"
...
if (-Not $RoleAss)
{
    Write-Warning -Message "Role assignment not found. Creating the role assignment for $AlyaWvdServicePrincipalNameProd"
