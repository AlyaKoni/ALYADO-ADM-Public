...
Start-Transcript -Path "$($AlyaLogs)\scripts\wvd\admin\prod\04_assignRoleToUsers-$($AlyaTimeString).log" | Out-Null
...
foreach($User in $Users)
{
    ...
    foreach($RoleName in $RoleNames)
    {
        ...
        if (-Not $RoleAss)
        {
            Write-Warning -Message "Role assignment not found. Creating the role assignment for $User"
