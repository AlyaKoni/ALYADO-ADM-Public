...
Start-Transcript -Path "$($AlyaLogs)\scripts\wvd\admin\test\07_createRdpHostPool_hpol003-$($AlyaTimeString).log" | Out-Null

# Constants
$ErrorActionPreference = "Stop"
$HostPoolName = "$($AlyaNamingPrefixTest)hpol003"
$ResourceGroupName = "$($AlyaNamingPrefixTest)resg053"
$NamePrefix = "$($AlyaNamingPrefixTest)vd53"
$ImageSourceName = "$($AlyaNamingPrefix)serv041_ImageServer"
...
$OuPath = "OU=TESTS,OU=WVD,OU=COMPUTERS,OU=CLOUD,DC=ALYACONSULTING,DC=LOCAL"
$ExistingVnetName = "$($AlyaNamingPrefixTest)vnet000"
$ExistingSubnetName = "$($AlyaNamingPrefixTest)vnet000snet01"
$virtualNetworkResourceGroupName = "$($AlyaNamingPrefixTest)resg000"
...
Write-Host "WVD | 07_createRdpHostPool_hpol003 | AZURE" -ForegroundColor $CommandInfo
