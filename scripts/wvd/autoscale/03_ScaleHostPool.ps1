        ...
        if ($NumberOfRunningHost -gt $MinimumNumberOfRDSH) {
          ...
          if (($SessionHostInfo.Status -eq "Available" -or $SessionHostInfo.Status -eq "NeedsAssistance")) {
            ...
            try {
              ...
              Write-Error "Unable to set it to allow connections on session host: $($SessionHost.SessionHost) with error: $($_.exception.message)"
              DoExit -exitCode 9
            }
              ...
              foreach ($Session in $HostPoolUserSessions) {
                if ($Session.SessionHostName -eq $SessionHost) {
                  ...
                  try {
                    ...
                    Write-Error "Failed to log off user with error: $($_.exception.message)"
                    DoExit -exitCode 13
                  }
                }
              }
...
if ($HostpoolInfo.LoadBalancerType -eq "BreadthFirst") {
  ...
  if ($CurrentDateTime -ge $BeginPeakDateTime -and $CurrentDateTime -le $EndPeakDateTime) {
    ...
    if ($AllSessionHosts -eq $null) {
      Write-Error "Sessionhosts does not exist in the Hostpool of '$HostpoolName'. Ensure that hostpool have hosts or not?."
      DoExit -exitCode 17
    }
          ...
          if ($SessionHost.ToLower().Contains($RoleInstance.Name.ToLower())) {
            ...
            if ($RoleInstance.PowerState -ne "VM running" -and $SessionHostInfo.UpdateState -eq "Succeeded") {
              ...
              try {
                ...
                Write-Error "Failed to start Azure VM: $($RoleInstance.Name) with error: $($_.exception.message)"
                DoExit -exitCode 19
              }
              ...
              if ($IsInstanceReady) {
                ...
                if ($ExistingSession -eq 0) {
                  ...
                  try {
                    ...
                    Write-Error "Failed to stop Azure VM: $($RoleInstance.Name) with error: $($_.exception.message)"
                    DoExit -exitCode 26
                  }
                  ...
                  if ($SessionHostInfo.UpdateState -eq "Succeeded") {
                    ...
                    try {
                      ...
                      Write-Error "Unable to set it to allow connections on session host: $($SessionHost | Out-String) with error: $($_.exception.message)"
                      DoExit -exitCode 27
                    }
                  }
                  $RoleSize = Get-AzVMSize -Location $RoleInstance.Location | Where-Object { $_.Name -eq $RoleInstance.HardwareProfile.VmSize }
