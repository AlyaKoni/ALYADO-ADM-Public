...
<#
.SYNOPSIS
Creates and configures an Azure Virtual Machine to function as an image host client for Azure Virtual Desktop environments.

.DESCRIPTION
The Create-ImageHostClient.ps1 script automates the creation and configuration of a Windows Virtual Desktop (WVD) image host virtual machine in Azure. It validates the required Azure resources such as resource groups, virtual networks, subnets, storage accounts, and key vaults, creating them if missing. It ensures necessary PowerShell Az modules are installed, handles Azure authentication, sets up VM credentials securely in Azure Key Vault, and deploys a virtual machine based on the defined naming conventions and parameters within the organization’s Azure environment. Logging is maintained for auditing and troubleshooting purposes.

.INPUTS
None. This script does not take pipeline input.

.OUTPUTS
None. The script performs Azure resource operations and produces log output. No objects are returned to the pipeline.

.EXAMPLE
PS> .\Create-ImageHostClient.ps1
Executes the script to create and configure an Azure Virtual Desktop image host virtual machine in accordance with the Alya Base Configuration.
...
if (-Not $ImageHostVm)
{
    ...
    $null = Set-AzResource -ResourceId $ImageHostVm.Id -Tag @{displayName="AVD Image Host";ownerEmail=$Context.Account.Id;stopTime=$AlyaWvdStopTime} -ApiVersion "2022-03-01" -Force
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAkC9cwdA+VTOIU
# OAq+efSJRDVTh3bq7Der+Jxrv0BG0qCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEICxECC+g
# YYttcLuTiA+9OpkzlKOOfZyZJc4XmnSGgs4FMA0GCSqGSIb3DQEBAQUABIICACfV
# X7yi0CL0kG8ackE/53+EM3acNCnLlf7OLDOp1Hr67ozf7JfAF9CVLr3o15IL/XLr
# vzyBiYNDC8/y7b45OfUd7/RUQSqJcE0+RkUs0FX4tLDuyh+pcHohDlBUicxpYRRW
# XNG3EkSOc1Q2qcsYHWLdvwPfG5aJoNvy2Lk1jIbd2sHEtGZhR/WAF8sF5Olt42FR
# jnr8riTUcTDCzEJb9cgYqf3S6Aqbms1jj3MBgv/KVhKdktGYUOUojiPsPk9evOgi
# /Fv8UM3ExRDYf+8xE0lHm2Y/XBM/ytUk7aehl0oxZEI5IRlsAlxIW0w2/WwC67d8
# 3WrA6xFD+a5Y6Wa/PCjOyCkynUWAqwO/qG0RrBUbp9oP51eeYAmcRdqCsVluUirO
# 5Hk0sw/avV61kprfW1EdHKMXK5MbQzvx1HAIV+iTqdVVfwz2Jd8quF99pMWWq/N9
# xuUIAHvtMSbAveGGDyTp12AKuJUgdsunO/O68pPjG8ys3mnpnOdq+eHu17EtRbQB
# nK/BMdT5JUpuWl1yv9Vki8W0Hpmd73WGvDDhj3I9MH6F6Q5Pc+yFTDCMh6TAZJut
# tVm70kdlbB9rERE7sY+vb6vGJizWnczxy0VMPET1na9HGNli+jQI/sdMkB84/AeI
# N9CbEaC6TaCU176kUcT3+VTrcmQAVw0GOs/jp5uRoYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCC9x/uI0Sz6zTpCybavrLfgFVc2Hp+d6UkIDI6pVlHE2AIUaX74
# DfiDhnzL+C9U+X1v/3yBrxMYDzIwMjYwMjEyMjAzMzQ3WjADAgEBoFikVjBUMQsw
...
# AQkEMSIEIOWjGTNI1iXcDecRUaFZQIG9Yu8l/duwrC4L9FUli1h5MIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAjCNq
# 7eCERRVHeZdXC9CDBHv5asRTA6NaqwZM9tuEQN85QSXuLVCYwEvtmj5LH71t28fZ
# NtkAeBh22N1H74GlH6WXetXipFF3V/+pXTRadnWTJSR3f0cRcNxngnr2cWM3xNzt
# w55hjqIiNuc7as5NIdocP/srPwp4EK2jnoSt8/NzF14zxxeEJRsLkgR8yoXEWo9L
# WRD1imxxX2Wovz4TNeTicLOG8WY1nFdtxCvpNWQM1PyQFp6chkhg1ahGYHJAw6R0
# J1oVyu62ydKup3EiI5aZoZEB51pSqW88l3mkxLxYctRmuRTsspG8NCd6Le2VQxSY
# xNe02gAnsOJossklXnirz+zZO/iVxlO6KuhzGK3ZscyEuh1qzVevq6QjNl4MFVEY
# ddcoU0zimmpD7YZXNHBodBKkajSoYFItNpxYNC3Dn1/0DhIqHuP/etdGqIbripIQ
# 2pvTn7Ao8YHFlleiGnm6UMNSVbqaK/lRGglC5AF/rJSP1DOHCqBz0CZeHM6c
