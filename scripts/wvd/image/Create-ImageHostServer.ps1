...
<#
.SYNOPSIS
Creates and configures an Azure Image Host Server virtual machine for the Alya Base Configuration environment.

.DESCRIPTION
The Create-ImageHostServer.ps1 script automates the creation and configuration of an Azure virtual machine used as an Image Host Server for Windows Virtual Desktop (WVD) or similar scenarios. It performs prerequisite checks, creates required Azure resources if absent, and configures networking, key vaults, credentials, diagnostic storage, and tagging. The script ensures that all components adhere to the Alya environment naming and configuration standards.

.INPUTS
None. The script relies on predefined environment variables and configuration values loaded from the Alya environment configuration file.

.OUTPUTS
Writes logs to the transcript file located in the Alya logs directory and outputs progress information to the console. Creates or updates Azure resources as needed.

.EXAMPLE
PS> .\Create-ImageHostServer.ps1
...
Start-Transcript -Path "$($AlyaLogs)\scripts\wvd\image\Create-ImageHostServer-$($AlyaTimeString).log" | Out-Null
...
if (-Not $ImageHostVm)
{
    ...
    $null = Set-AzResource -ResourceId $ImageHostVm.Id -Tag @{displayName="Image Host Server";ownerEmail=$Context.Account.Id;stopTime=$AlyaWvdStopTime} -ApiVersion "2022-03-01" -Force
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCOXCjT09ihrT83
# L/n7r1NHB5iO698tqLquz3VnPS5sKKCCFIswggWiMIIEiqADAgECAhB4AxhCRXCK
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIMtjR7zJ
# PkIEha0bLIEPH76R9vxwBVPlfSO2UOQK1kzfMA0GCSqGSIb3DQEBAQUABIICAA/w
# c7V1F5KkTISZnQCWVM1RQJ2vTuo8gI8M/gWhnM/8jgOjZSUxl9kbQEpYE+Zo6Igh
# AEb4/yFffubo2bNn93yg2yKmlrqDP+zV59y44Af/nq7gkECw+dV5Dm28cnrMvBVJ
# CnzeTTbxpl/9I9wSuX0ljfjbVdXqLuxn5VzljvbxTqAKm25tZIClY/RvjtTwIRx+
# Pv0nyDhwA57BTSrSY8nIU7HK9XWPpOPqO7JUSKBeYhaEvcLTAa7WO0cnY6lhx3Fy
# AsZNEVY+Q+FDL7Kh+BqNlGs6OQGiqjxLUBKb/ptWJDRxoyQ0Wp6Pfagrew3yGZHv
# bgULkKcEeo7+pCDLjLkyrWDLfP1WZnavSJX552YFzmkEPReoXbcWIaw+Hn0tDFzC
# XvJ72QrLhx26A/u4eiH8bFUbcS6E7owe3gAIW5/pDmQf44G4/GAba/kPUysHKrp5
# 6Pkgn72/YXHgZYmBWlfA/el3HO4j09ptfHEUPJR21sXcKzVaetZSWZ/sBI3zJJOY
# QnDu8fMHIUJY4L4G2q8F8tmgCQh08wvh9K97XWuRzy+6tS6pJQbmRy3OoRFIPsV/
# upKZDzxiOsl8MLPSsINY2xWtn5IVigOvtaqJ2NtAJZpVd98nbHpxHrnb0LwOj/Q1
# MEVeEd7JRn9FgGakQNaXd32yj20wUWD7CBsYxJOVoYIWuzCCFrcGCisGAQQBgjcD
...
# SAFlAwQCAQUABCAo2YH0DQAyKY78+l3n1OlPWZIGxazMWKz+qBdvMrq30AIULMxZ
# BvNpWgQ++FFjZ34RK2u1WkwYDzIwMjYwMjEyMjAzNDAyWjADAgEBoFikVjBUMQsw
...
# AQkEMSIEIDGqeRV+vPI9tSePxpS0Py/3lRF1z6NOK2WRr5LqbyHjMIGwBgsqhkiG
...
# Mzg0IC0gRzQCEAEACyAFs5QHYts+NnmUm6kwDQYJKoZIhvcNAQELBQAEggGAbSPZ
# ImeDlYCgoToFvrHNIYoPK5lPubwV7i4kY1999p9gFsGHrJHGP6SMQco+E9ic5GuG
# 4VEgSWpx7BkEjRSNv7G3Cmomd0xPMFTCrdLw2WU0CU54oLdMat8u0kgg/7AqeY5O
# metFxpAKxgmwcXV/nNqO0HTpAjomPAdoELNBTVjVZX5YzMGbxqit4JGnp54ySR9D
# pru04k7xSj2qC1lKoHEmSdjIxRsMw07HISy4uEFPftpK+DDP9ZearV9dFF9j/yC9
# h8WwbtDnDIMwnaRgoHOJ4r8o59IBCs9qJsxwO72FjOxF/+lMFFuwOt2YZlg0/49i
# i5rQn5HJRKW2dfjz23rMqgBKsz8dfs3TeIKlf+3AUeXIUjcf6753t0Gb2MlNv+UQ
# oDvbEEBpPIxoszFm2vxykK9HeeWM5UmUZhebGaHDeb3Mh64C2/pTojaic0avf++3
# lYhhWFUCCduTNoIGB+5taQH0qi5wl4fiCUiY6z50icIWOBL0/0wcyEASy4xm
